    ...
    if ($_REQUEST['day_re_assignment'] != '' && $_REQUEST['month_re_assignment'] != '' && $_REQUEST['year_re_assignment'] != '') {
        if (strtotime($assign_date) >= strtotime(date('Y-m-d'))) {
            if (scheduleAssociation($id)) {
                ...
                ShowErrPhp('' . _thereIsNoAssociationsInHisCoursePeriodYouCanDeleteItFromInstituteSetUpCourseManager . '');
            }
        } else {
...
if ((!$_REQUEST['modfunc'] || clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'choose_course') && !$_REQUEST['course_modfunc']) {
    ...
    if ($_REQUEST['modfunc'] != 'choose_course') {
        if (clean_param($_REQUEST['course_period_id'], PARAM_ALPHANUM)) {
            ...
            $teachers_RET = DBGet(DBQuery('SELECT STAFF_ID,LAST_NAME,FIRST_NAME,MIDDLE_NAME FROM staff st INNER JOIN staff_institute_relationship ssr USING (staff_id) WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND PROFILE=\'teacher\' AND staff_id <>\'' . $RET['TEACHER_ID'] . '\' AND (IS_DISABLE IS NULL OR IS_DISABLE<>\'Y\')  ORDER BY LAST_NAME,FIRST_NAME '));
