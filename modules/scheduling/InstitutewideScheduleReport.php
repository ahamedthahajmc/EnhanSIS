...
$get_institute_days = DBGet(DBQuery("SELECT days FROM `institute_calendars` WHERE `syear` = '".UserSyear()."' AND `institute_id` = '".UserInstitute()."' AND `default_calendar` = 'Y'"));

$instituteing_days = $get_institute_days[1]['DAYS'];
$instituteing_days_arr = str_split($instituteing_days);

$first_day_name =   GetDaysNames(array_values($instituteing_days_arr)[0]);
$end_day_name   =   GetDaysNames(end($instituteing_days_arr));
...
$get_periods = DBGet(DBQuery("SELECT period_id, title, short_name, start_time, end_time FROM `institute_periods` WHERE `syear` = '".UserSyear()."' AND `institute_id` = '".UserInstitute()."' ORDER BY `sort_order`"));

$get_subjects = DBGet(DBQuery("SELECT subject_id, title FROM `course_subjects` WHERE `institute_id` = '".UserInstitute()."' AND syear = '".UserSyear()."' ORDER BY `subject_id`"));
...
echo '<ul class="nav nav-tabs nav-tabs-bottom no-margin-bottom"><li class="active"><a href="javascript:void(0);">'._institutewideScheduleReport.'</a></li></ul>';
...
foreach($this_week as $each_day)
{
    ...
    if(in_array($this_day_shortname, $instituteing_days_arr))
    {
    	echo '<li class="'.$this_active.'"><a onclick="getInstituteScheduleReport(\''.$day_inc.'\');" href="javascript:void(0);">'.date('l', strtotime($each_day)).'</a></li>';
...
if(!empty($get_institute_days) && $get_institute_days != '')
{
...
{
    echo '<div class="alert alert-danger alert-bordered">'._instituteCalendarIsNotSet.'</div>';
}
