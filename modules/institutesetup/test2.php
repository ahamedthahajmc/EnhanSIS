 ...
 */
  ...
  $colmn = Calender_Id;
    ...
    $day_RET = DBGet(DBQuery('SELECT DAYNAME(INSTITUTE_DATE) AS DAY_NAME FROM attendance_calendar WHERE CALENDAR_ID=\'' . $cal_id . '\' ORDER BY INSTITUTE_DATE LIMIT 0, 7'));
    $i = 0;
    ...
    $message .= '<div class="checkbox"><label><INPUT type=checkbox name=default value=Y ' . (($acs_RET['DEFAULT_CALENDAR'] == 'Y') ? 'checked' : '') . '> Default Calendar for this Institute</label></div>';
    $message .= '</div>';
