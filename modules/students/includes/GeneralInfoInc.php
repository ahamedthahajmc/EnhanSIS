...
echo '<div class="row">';
echo '<div class="col-lg-6">';
echo '<div class="form-group">' . TextInput($student['EMIS_NO'], 'students[EMIS_NO]', '' . _emisNo . '', 'size=8 class=cell_medium maxlength=100') . '</div>';
echo '</div>'; //.col-lg-6
echo '</div>'; //.row
...
echo '<input type="checkbox" id="RTE" name="students[RTE]" value="1" ' . ($student['RTE'] == 1 ? 'checked' : 'unchecked') . ' /> ' . _RTE;
echo '</div>';
echo '</div>'; //.col-lg-6
echo '</div>'; //.row
...
echo '<h5 class="pt-20 text-primary">' . _instituteInformation . '</h5>';
...
if ($_REQUEST['student_id'] != 'new' && $student['INSTITUTE_ID'])
    $institute_id = $student['INSTITUTE_ID'];
else
    $institute_id = UserInstitute();
$sql = 'SELECT ID,TITLE FROM institute_gradelevels WHERE INSTITUTE_ID=\'' . $institute_id . '\' ORDER BY SORT_ORDER';
$QI = DBQuery($sql);
$grades_RET = DBGet($QI);
unset($options);
if (count($grades_RET)) {
...
if ($_REQUEST['student_id'] != 'new' && $student['INSTITUTE_ID'] != UserInstitute()) {
    $allow_edit = $_hani['allow_edit'];
    $AllowEdit = $_hani['AllowEdit'][$_REQUEST['modname']];
    $_hani['AllowEdit'][$_REQUEST['modname']] = $_hani['allow_edit'] = false;
}
...
if (UserStudentID()) {
    $stu_img_info = DBGet(DBQuery('SELECT * FROM user_file_upload WHERE USER_ID=' . UserStudentID() . ' AND PROFILE_ID=3 AND INSTITUTE_ID=' . UserInstitute() . ' AND FILE_INFO=\'stuimg\' AND ID = (SELECT MAX(ID) AS LATEST_UPLOADED_IMG FROM user_file_upload WHERE USER_ID=' . UserStudentID() . ' AND PROFILE_ID=3)')); //  AND SYEAR=' . UserSyear() . '
}
