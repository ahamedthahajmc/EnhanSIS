...
if($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax']) && AllowEdit())
{
	foreach($_REQUEST['attendance'] as $student_id=>$values)
	{
		foreach($values as $institute_date=>$columns)
		{
			...
			$sql = substr($sql,0,-1) . ' WHERE INSTITUTE_DATE=\''.$institute_date.'\' AND PERIOD_ID=\''.$_REQUEST['period_id'].'\' AND STUDENT_ID=\''.$student_id.'\'';
			DBQuery($sql);
			UpdateAttendanceDaily($student_id,$institute_date);
		}
	}
	$current_RET = DBGet(DBQuery('SELECT ATTENDANCE_TEACHER_CODE,ATTENDANCE_CODE,ATTENDANCE_REASON,STUDENT_ID,ADMIN,COURSE_PERIOD_ID FROM attendance_period WHERE INSTITUTE_DATE=\''.$date.'\''),array(),array('STUDENT_ID','COURSE_PERIOD_ID'));
...
$cal_RET = DBGet(DBQuery('SELECT DISTINCT INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(INSTITUTE_DATE,\'%y%b%d\')) AS SHORT_DATE FROM attendance_calendar WHERE INSTITUTE_ID=\''.UserInstitute().'\' AND INSTITUTE_DATE BETWEEN \''.date('Y-m-d',strtotime($start_date)).'\' AND \''.date('Y-m-d',strtotime($end_date)).'\' ORDER BY INSTITUTE_DATE'));
...
if(UserStudentID() || $_REQUEST['student_id'] || User('PROFILE')=='parent')
{
	...
	if($_REQUEST['period_id'])
	{
		$sql = 'SELECT
			...
			FROM
				schedule s,courses c,course_periods cp,course_period_var cpv,institute_periods sp
			WHERE
		...
		$sql = 'SELECT ap.INSTITUTE_DATE,ap.PERIOD_ID,ac.SHORT_NAME,ac.STATE_CODE,ac.DEFAULT_CODE FROM attendance_period ap,attendance_codes ac WHERE ap.INSTITUTE_DATE BETWEEN \''.$start_date.'\' AND \''.$end_date.'\' AND ap.ATTENDANCE_CODE=ac.ID AND ap.STUDENT_ID=\''.UserStudentID().'\'';
		$attendance_RET = DBGet(DBQuery($sql),array(),array('INSTITUTE_DATE','PERIOD_ID'));
	}
	else
	{
		$schedule_RET[1] = array('COURSE_PERIOD'=>'Daily Attendance','PERIOD_ID'=>'0');
		$attendance_RET = DBGet(DBQuery('SELECT ad.INSTITUTE_DATE,\'0\' AS PERIOD_ID,ad.STATE_VALUE AS STATE_CODE,'.db_case(array('ad.STATE_VALUE',"'0.0'","'A'","'1.0'","'P'","'H'")).' AS SHORT_NAME FROM attendance_day ad WHERE ad.INSTITUTE_DATE BETWEEN \''.$start_date.'\' AND \''.$end_date.'\' AND ad.STUDENT_ID=\''.UserStudentID().'\''),array(),array('INSTITUTE_DATE','PERIOD_ID'));
	}
	...
	if(count($schedule_RET))
	{
		foreach($schedule_RET as $course)
		{
			...
			foreach($cal_RET as $value)
				$student_RET[$i][$value['SHORT_DATE']] = _makePeriodColor($attendance_RET[$value['INSTITUTE_DATE']][$course['PERIOD_ID']][1]['SHORT_NAME'],$attendance_RET[$value['INSTITUTE_DATE']][$course['PERIOD_ID']][1]['STATE_CODE'],$attendance_RET[$value['INSTITUTE_DATE']][$course['PERIOD_ID']][1]['DEFAULT_CODE']);
		}
	}
...
else
{
	if(!$_REQUEST['period_id'])
	{
			$sql = 'SELECT ad.STATE_VALUE,ad.STUDENT_ID,INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ad.INSTITUTE_DATE,\'%y%b%d\')) AS SHORT_DATE FROM attendance_day ad,student_enrollment ssm WHERE ad.STUDENT_ID=ssm.STUDENT_ID AND ((\''.DBDate().'\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR ssm.END_DATE IS NULL) AND \''.DBDate().'\'>=ssm.START_DATE) AND ssm.INSTITUTE_ID=\''.UserInstitute().'\' AND INSTITUTE_DATE BETWEEN  \''.date('Y-m-d',strtotime($start_date)).'\' AND \''.date('Y-m-d',strtotime($end_date)).'\'';
		$RET = DBGet(DBQuery($sql),array(),array('STUDENT_ID','SHORT_DATE'));
	}
	else
	{
		$sql = 'SELECT ap.ATTENDANCE_CODE,ap.STUDENT_ID,ap.INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ap.INSTITUTE_DATE,\'%y%b%d\')) AS SHORT_DATE FROM attendance_period ap,student_enrollment ssm,students s WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ap.STUDENT_ID=ssm.STUDENT_ID AND ap.INSTITUTE_DATE BETWEEN  \''.date('Y-m-d',strtotime($start_date)).'\' AND \''.date('Y-m-d',strtotime($end_date)).'\'';
...
{	global $THIS_RET,$RET,$attendance_codes;
	...
	{
		if(!$attendance_codes)
			$attendance_codes = DBGet(DBQuery('SELECT ID,DEFAULT_CODE,STATE_CODE,SHORT_NAME FROM attendance_codes WHERE SYEAR=\''.UserSyear().'\' AND INSTITUTE_ID=\''.UserInstitute().'\' AND TABLE_NAME=\'0\''),array(),array('ID'));
...
{	global $THIS_RET,$attendance_codes,$_HaniIMSIMS;
	...
	if(!$_HaniIMS['code_options'])
	{
		foreach($attendance_codes as $id=>$code)
			$_HaniIMS['code_options'][$id] = $code[1]['SHORT_NAME'];
	}
