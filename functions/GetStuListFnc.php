...
{
    global $contacts_RET, $view_other_RET, $_HaniIMS, $select;
    $offset = 'GRADE_ID';
    ...
    if ($_REQUEST['expanded_view'] == 'true') {
        ...
        if (!count($view_fields_RET) && !isset($view_address_RET) && !isset($view_other_RET['CONTACT_INFO'])) {
            ...
            if ($view_other_RET['CONTACT_INFO'][1]['VALUE'] == 'Y' && !$_REQUEST['HaniIMS_PDF']) {
    ...
    $stu_arr = array();
    if (($_REQUEST['include_inactive'] == 'Y' && $_REQUEST['_search_all_institutes'] == 'Y'))
        $tot_stu = DBGet(DBQuery('SELECT STUDENT_ID , ID FROM student_enrollment WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID IN (' . GetUserInstitutes(UserID(), true) . ') ORDER BY START_DATE DESC'));
    if (($_REQUEST['include_inactive'] == 'Y' && $_REQUEST['_search_all_institutes'] != 'Y'))
        $tot_stu = DBGet(DBQuery('SELECT STUDENT_ID , ID FROM student_enrollment WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY START_DATE DESC'));
    $tot_stu_id = '';
    ...
    switch (User('PROFILE')) {
        case 'admin':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,ssm.INSTITUTE_ID,s.ALT_ID,ssm.INSTITUTE_ID AS LIST_INSTITUTE_ID,ssm.GRADE_ID' . $extra['SELECT'];
            ...
            if ($_REQUEST['modname'] != 'students/StudentReenroll.php') {
                ...
                if (UserInstitute() && $_REQUEST['_search_all_institutes'] != 'Y') {
                    $sql .= ' AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\'';
                    $allSQL .= ' AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\'';
                } else {
                    $sql .= ' AND ssm.INSTITUTE_ID IN (' . GetUserInstitutes(UserID(), true) . ') ';
                    $allSQL .= ' AND ssm.INSTITUTE_ID IN (' . GetUserInstitutes(UserID(), true) . ') ';
                    $extra['columns_after']['LIST_INSTITUTE_ID'] = 'Institute';
                    $functions['LIST_INSTITUTE_ID'] = 'GetInstitute';
                }
                ...
                if ($_REQUEST['_search_all_institutes'] == 'Y') {
                    ...
                    $sql .= ' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\'';
                    $allSQL .= ' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\'';
                }
            }
        ...
        case 'teacher':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                if (Preferences('NAME') == 'Common')
                    ...
                    $sql .= 'CONCAT(s.LAST_NAME,\', \',s.FIRST_NAME,\' \',COALESCE(s.MIDDLE_NAME,\' \')) AS FULL_NAME,';
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,s.ALT_ID,ssm.INSTITUTE_ID,ssm.GRADE_ID ' . $extra['SELECT'];
            ...
            $sql .= $extra['FROM'] . ' WHERE ssm.STUDENT_ID=s.STUDENT_ID AND ssm.STUDENT_ID=ss.STUDENT_ID
					AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.SYEAR=cp.SYEAR AND ssm.SYEAR=ss.SYEAR
            ...
            if (!$extra['SELECT_ONLY'] && $_REQUEST['include_inactive'] == 'Y') {
                $extra['columns_after']['ACTIVE'] = 'Institute Status';
        ...
        case 'student':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                if (Preferences('NAME') == 'Common')
                    ...
                    $sql .= 'CONCAT(s.LAST_NAME,\', \',s.FIRST_NAME,\' \',COALESCE(s.MIDDLE_NAME,\' \')) AS FULL_NAME,';
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.ALT_ID,ssm.INSTITUTE_ID,ssm.GRADE_ID ' . $extra['SELECT'];
            }
            if ($_REQUEST['modname'] == 'grades/GPARankList.php') {
                $sql .= ' FROM students s,student_enrollment ssm ' . $extra['FROM'] . '
					WHERE ssm.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND (\'' . DBDate() . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND \'' . DBDate() . '\'>=ssm.START_DATE))';
                $allSQL .= ' FROM students s,student_enrollment ssm ' . $extra['FROM'] . '
                    WHERE ssm.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND (\'' . DBDate() . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND \'' . DBDate() . '\'>=ssm.START_DATE))';
            } else {
                $sql .= ' FROM students s,student_enrollment ssm ' . $extra['FROM'] . '
					WHERE ssm.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND (\'' . DBDate() . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND \'' . DBDate() . '\'>=ssm.START_DATE)) AND ssm.STUDENT_ID' . ($extra['ASSOCIATED'] ? ' IN (SELECT STUDENT_ID FROM students_join_people WHERE PERSON_ID=\'' . $extra['ASSOCIATED'] . '\')' : '=\'' . UserStudentID() . '\'');
                $allSQL .= ' FROM students s,student_enrollment ssm ' . $extra['FROM'] . '
                    WHERE ssm.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND (\'' . DBDate() . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND \'' . DBDate() . '\'>=ssm.START_DATE)) AND ssm.STUDENT_ID' . ($extra['ASSOCIATED'] ? ' IN (SELECT STUDENT_ID FROM students_join_people WHERE PERSON_ID=\'' . $extra['ASSOCIATED'] . '\')' : '=\'' . UserStudentID() . '\'');
            }
            break;
    ...
    if (!$extra['ORDER_BY'] && !$extra['SELECT_ONLY']) {
        if (Preferences('SORT') == 'Grade')
            $sql .= ' ORDER BY (SELECT SORT_ORDER FROM institute_gradelevels WHERE ID=ssm.GRADE_ID),FULL_NAME';
        else
    ...
    if ($extra['LIMIT']) {
        ...
        if ((!$_REQUEST['HaniIMS_PDF'] || $_REQUEST['HaniIMS_PDF'] == false) && !checkNoNeedPaging($_REQUEST['modname'])) {
            $sql = $sql . ' LIMIT 0,50';
        }
    }
...
{
    global $THIS_RET, $view_other_RET, $_HaniIMS;
    ...
    if ($THIS_RET['PARENTS'] == $student_id) {
        ...
        if ($_HaniIMS['makeParents'])
            $constraint = 'AND (LOWER(sjp.RELATIONSHIP) LIKE \'' . strtolower($_HaniIMS['makeParents']) . '%\')';
...
{
    global $_HaniIMS, $imm_date, $med_date, $ma_date, $nv_date;

    if ($_REQUEST['stuid']) {
        $sql .= ' AND ssm.STUDENT_ID = \'' . singleQuoteReplace("'", "\'", $_REQUEST['stuid']) . '\' ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Student ID: </b></font>' . $_REQUEST['stuid'] . '<BR>';
    }
    if ($_REQUEST['altid']) {
    ...
    if ($_REQUEST['last']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Last Name starts with: </b></font>' . stripslashes(trim($_REQUEST['last'])) . '<BR>';
    }
    if ($_REQUEST['first']) {
        $sql .= ' AND LOWER(s.FIRST_NAME) LIKE \'' . singleQuoteReplace("'", "\'", strtolower(trim($_REQUEST['first']))) . '%\' ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>First Name starts with: </b></font>' . stripslashes(trim($_REQUEST['first'])) . '<BR>';
    }
    if ($_REQUEST['grade']) {
        $sql .= ' AND ssm.GRADE_ID IN(SELECT id FROM institute_gradelevels WHERE id= \'' . singleQuoteReplace("'", "\'", $_REQUEST['grade']) . '\')';
        if (!$extra['NoSearchTerms']) {
            $title = DBGet(DBQuery('SELECT title FROM institute_gradelevels WHERE id= \'' . singleQuoteReplace("'", "\'", $_REQUEST['grade']) . '\''));
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Grade: </b></font>' . $title[1]['TITLE'] . '<BR>';
	    }
    }
    if ($_REQUEST['addr']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Address contains: </b></font>' . trim($_REQUEST['addr']) . '<BR>';
    }
    if ($_REQUEST['preferred_hospital']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Preferred Medical Facility starts with: </b></font>' . $_REQUEST['preferred_hospital'] . '<BR>';
    }
    ...
    if ($_REQUEST['mp_comment']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Comments starts with: </b></font>' . $_REQUEST['mp_comment'] . '<BR>';
    }
    if ($_REQUEST['goal_title']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>GoalInc Title starts with: </b></font>' . $_REQUEST['goal_title'] . '<BR>';
    }
    ...
    if ($_REQUEST['goal_description']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>GoalInc Description starts with: </b></font>' . $_REQUEST['goal_description'] . '<BR>';
    }
    if ($_REQUEST['progress_name']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Progress Period Name starts with: </b></font>' . $_REQUEST['progress_name'] . '<BR>';
    }
    if ($_REQUEST['progress_description']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Progress Assessment starts with: </b></font>' . $_REQUEST['progress_description'] . '<BR>';
    }
    if ($_REQUEST['doctors_note_comments']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Doctor\'s Note starts with: </b></font>' . $_REQUEST['doctors_note_comments'] . '<BR>';
    }
    if ($_REQUEST['type']) {
        $sql .= ' AND LOWER(sm.TYPE) LIKE \'' . singleQuoteReplace("'", "\'", strtolower($_REQUEST['type'])) . '%\' AND s.STUDENT_ID=sm.STUDENT_ID ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Type starts with: </b></font>' . $_REQUEST['type'] . '<BR>';
    }
    if ($_REQUEST['imm_comments']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Comments starts with: </b></font>' . $_REQUEST['imm_comments'] . '<BR>';
    }
    if ($_REQUEST['imm_day'] && $_REQUEST['imm_month'] && $_REQUEST['imm_year']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Immunization Date: </b></font>' . $imm_date . '<BR>';
    ...
    if ($_REQUEST['med_day'] && $_REQUEST['med_month'] && $_REQUEST['med_year']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Medical Date: </b></font>' . $med_date . '<BR>';
    ...
    if ($_REQUEST['ma_day'] && $_REQUEST['ma_month'] && $_REQUEST['ma_year']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Medical Alert Date: </b></font>' . $ma_date . '<BR>';
    ...
    if ($_REQUEST['nv_day'] && $_REQUEST['nv_month'] && $_REQUEST['nv_year']) {
        ...
        $sql .= ' AND smv.INSTITUTE_DATE =\'' . date('Y-m-d', strtotime($nv_date)) . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Nurse Visit Date: </b></font>' . $nv_date . '<BR>';
    } elseif ($_REQUEST['nv_day'] || $_REQUEST['nv_month'] || $_REQUEST['nv_year']) {
        if ($_REQUEST['nv_day']) {
            $sql .= ' AND SUBSTR(smv.INSTITUTE_DATE,9,2) =\'' . $_REQUEST['nv_day'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Day :" . $_REQUEST['nv_day'];
        }
        if ($_REQUEST['nv_month']) {
            $sql .= ' AND SUBSTR(smv.INSTITUTE_DATE,6,2) =\'' . $_REQUEST['nv_month'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Month :" . $_REQUEST['nv_month'];
        }
        if ($_REQUEST['nv_year']) {
            $sql .= ' AND SUBSTR(smv.INSTITUTE_DATE,1,4) =\'' . $_REQUEST['nv_year'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
    ...
    if ($_REQUEST['med_alrt_title']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Alert starts with: </b></font>' . $_REQUEST['med_alrt_title'] . '<BR>';
    }
    if ($_REQUEST['reason']) {
        $sql .= ' AND LOWER(smv.REASON) LIKE \'' . singleQuoteReplace("'", "\'", strtolower($_REQUEST['reason'])) . '%\' AND s.STUDENT_ID=smv.STUDENT_ID ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Reason starts with: </b></font>' . $_REQUEST['reason'] . '<BR>';
    }
    if ($_REQUEST['result']) {
        $sql .= ' AND LOWER(smv.RESULT) LIKE \'' . singleQuoteReplace("'", "\'", strtolower($_REQUEST['result'])) . '%\' AND s.STUDENT_ID=smv.STUDENT_ID ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Result starts with: </b></font>' . $_REQUEST['result'] . '<BR>';
    }
    if ($_REQUEST['med_vist_comments']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Nurse Visit Comments starts with: </b></font>' . $_REQUEST['med_vist_comments'] . '<BR>';
    }
    ...
    if ($_REQUEST['day_to_birthdate'] && $_REQUEST['month_to_birthdate']  && $_REQUEST['day_from_birthdate'] && $_REQUEST['month_from_birthdate']) {
        ...
        $sql .= ' AND (SUBSTR(s.BIRTHDATE,6,2) BETWEEN \'' . $_REQUEST['month_from_birthdate'] . '\' AND \'' . $_REQUEST['month_to_birthdate'] . '\') ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Birthday Starts from ' . $date_from . ' to ' . $date_to . '</b></font>';
    }
    ...
    if ($_REQUEST['day_dob_birthdate'] && $_REQUEST['month_dob_birthdate']  && $_REQUEST['year_dob_birthdate']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Birthday is ' . $date_dob . '</b></font>';
    }
    ...
    if ($_REQUEST['day_to_est'] && $_REQUEST['month_to_est'] && $_REQUEST['day_from_est'] && $_REQUEST['month_from_est']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Estimated Grad. Date Starts from ' . $date_from_est . ' to ' . $date_to_est . '</b></font>';
    }
    ...
    if ($_REQUEST['day_to_st'] && $_REQUEST['month_to_st'] && $_REQUEST['day_from_st'] && $_REQUEST['month_from_st']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Enrollment Starts from ' . $date_from_st . ' to ' . $date_to_st . '</b></font>';
    }
    ...
    if ($_REQUEST['day_to_en'] && $_REQUEST['month_to_en'] && $_REQUEST['day_from_en'] && $_REQUEST['month_from_en']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Enrollment Ends from ' . $date_from_en . ' to ' . $date_to_en . '</b></font>';
    }
    // test cases start
    // test cases end
...
function appendAllSQL($allSQL, &$extra)
{
    ...
    if ($_REQUEST['grade']) {
        ...
        $allSQL .= ' AND ssm.GRADE_ID IN(SELECT id FROM institute_gradelevels WHERE title= \'' . singleQuoteReplace("'", "\'", $grade) . '\')';
    }
    if ($_REQUEST['addr']) {
    ...
    if ($_REQUEST['nv_day'] && $_REQUEST['nv_month'] && $_REQUEST['nv_year']) {
        $nv_date = $_REQUEST['nv_year'] . '-' . $_REQUEST['nv_month'] . '-' . $_REQUEST['nv_day'];
        $allSQL .= ' AND smv.INSTITUTE_DATE =\'' . date('Y-m-d', strtotime($nv_date)) . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
    } elseif ($_REQUEST['nv_day'] || $_REQUEST['nv_month'] || $_REQUEST['nv_year']) {
        if ($_REQUEST['nv_day']) {
            $allSQL .= ' AND SUBSTR(smv.INSTITUTE_DATE,9,2) =\'' . $_REQUEST['nv_day'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Day :" . $_REQUEST['nv_day'];
        }
        if ($_REQUEST['nv_month']) {
            $allSQL .= ' AND SUBSTR(smv.INSTITUTE_DATE,6,2) =\'' . $_REQUEST['nv_month'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Month :" . $_REQUEST['nv_month'];
        }
        if ($_REQUEST['nv_year']) {
            $allSQL .= ' AND SUBSTR(smv.INSTITUTE_DATE,1,4) =\'' . $_REQUEST['nv_year'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Year :" . $_REQUEST['nv_year'];
        }
    }
...
function GetStuList_Absence_Summary(&$extra)
{
    ...
    switch (User('PROFILE')) {
        case 'admin':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,ssm.INSTITUTE_ID,s.ALT_ID,ssm.INSTITUTE_ID AS LIST_INSTITUTE_ID,ssm.GRADE_ID' . $extra['SELECT'];
                $_SESSION['new_sql'] .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,ssm.INSTITUTE_ID,s.ALT_ID,ssm.INSTITUTE_ID AS LIST_INSTITUTE_ID,ssm.GRADE_ID' . $_SESSION['new_customsql'];
                //                if ($_REQUEST['include_inactive'] == 'Y')
            ...
            if (UserInstitute() && $_REQUEST['_search_all_institutes'] != 'Y') {
                $sql .= ' AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\'';
                $_SESSION['new_sql'] .= ' AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\'';
            } else {
                $sql .= ' AND ssm.INSTITUTE_ID IN (' . GetUserInstitutes(UserID(), true) . ') ';
                $_SESSION['new_sql'] .= ' AND ssm.INSTITUTE_ID IN (' . GetUserInstitutes(UserID(), true) . ') ';
        ...
        case 'teacher':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                $_SESSION['new_sql'] = $sql;
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,s.ALT_ID,ssm.INSTITUTE_ID,ssm.GRADE_ID ' . $extra['SELECT'];
                $_SESSION['new_sql'] .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,s.ALT_ID,ssm.INSTITUTE_ID,ssm.GRADE_ID ' . $_SESSION['new_customsql'];
                if ($_REQUEST['include_inactive'] == 'Y') {
    ...
    if ($extra['LIMIT']) {
        ...
        if (!$_REQUEST['HaniIMS_PDF'] || $_REQUEST['HaniIMS_PDF'] == false) {
            $sql = $sql . ' LIMIT 0,50';
        }
    }
...
{
    global $_HaniIMS, $imm_date, $med_date, $nv_date, $ma_date;
    if ($_REQUEST['stuid']) {
        $sql .= ' AND ssm.STUDENT_ID = \'' . singleQuoteReplace("'", "\'", $_REQUEST['stuid']) . '\' ';
    ...
    if ($_REQUEST['grade']) {
        ...
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Grade: </b></font>' . GetGrade($_REQUEST['grade']) . '<BR>';
    }
    if ($_REQUEST['addr']) {
    ...
    if ($_REQUEST['nv_day'] && $_REQUEST['nv_month'] && $_REQUEST['nv_year']) {
        $nv_date = $_REQUEST['nv_year'] . '-' . $_REQUEST['nv_month'] . '-' . $_REQUEST['nv_day'];
        $sql .= ' AND smv.INSTITUTE_DATE =\'' . date('Y-m-d', strtotime($nv_date)) . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
        $_SESSION['newsql1'] .= ' AND smv.INSTITUTE_DATE =\'' . date('Y-m-d', strtotime($nv_date)) . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Nurse Visit Date: </b></font>' . $nv_date . '<BR>';
    } elseif ($_REQUEST['nv_day'] || $_REQUEST['nv_month'] || $_REQUEST['nv_year']) {
        if ($_REQUEST['nv_day']) {
            $sql .= ' AND SUBSTR(smv.INSTITUTE_DATE,9,2) =\'' . $_REQUEST['nv_day'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $_SESSION['newsql1'] .= ' AND SUBSTR(smv.INSTITUTE_DATE,9,2) =\'' . $_REQUEST['nv_day'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Day :" . $_REQUEST['nv_day'];
        }
        if ($_REQUEST['nv_month']) {
            $sql .= ' AND SUBSTR(smv.INSTITUTE_DATE,6,2) =\'' . $_REQUEST['nv_month'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $_SESSION['newsql1'] .= ' AND SUBSTR(smv.INSTITUTE_DATE,6,2) =\'' . $_REQUEST['nv_month'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Month :" . $_REQUEST['nv_month'];
        }
        if ($_REQUEST['nv_year']) {
            $sql .= ' AND SUBSTR(smv.INSTITUTE_DATE,1,4) =\'' . $_REQUEST['nv_year'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $_SESSION['newsql1'] .= ' AND SUBSTR(smv.INSTITUTE_DATE,1,4) =\'' . $_REQUEST['nv_year'] . '\' AND s.STUDENT_ID=smv.STUDENT_ID ';
            $nv_date .= " Year :" . $_REQUEST['nv_year'];
        }
        if (!$extra['NoSearchTerms'])
            $_HaniIMS['SearchTerms'] .= '<font color=gray><b>Nurse Visit Date: </b></font>' . $nv_date . '<BR>';
...
function _make_Parents($value, $column = '')
{
    ...
    $sql = 'SELECT DISTINCT person_id AS STAFF_ID, CONCAT( people.LAST_NAME, \' \', people.FIRST_NAME ) AS PARENT FROM students_join_people sju, people, staff_institute_relationship ssr WHERE people.staff_id = sju.person_id and sju.student_id=\'' . $value . '\' AND ssr.syear=\'' . UserSyear() . '\'';
