...
function VerifyFixedSchedule($columns, $columns_var, $update = false)
{
    ...
    $min_start_dt_chk_qr = DBGet(DBQuery("SELECT min(start_date) as start_date from  staff_institute_relationship where staff_id in($all_teacher)"));
    $min_start_dt_chk_pr = DBGet(DBQuery("SELECT min(start_date) as start_date from  staff_institute_relationship where staff_id in($teacher)"));
    ...
    $period_time = DBGet(DBQuery("SELECT START_TIME, END_TIME ,IGNORE_SCHEDULING FROM institute_periods WHERE period_id={$period}"));
    ...
    // $period_append_sql = " AND period_id IN(SELECT period_id from institute_periods WHERE start_time<='$end_time' AND '$start_time'<=end_time)";
    $period_append_sql = " AND period_id IN(SELECT period_id from institute_periods WHERE start_time >= '$start_time' AND end_time <= '$end_time')";
...
function VerifyVariableSchedule($columns)
{
    ...
    foreach ($_REQUEST['course_period_variable'] as $cp_id => $days) {
        ...
        $period_time = DBGet(DBQuery("SELECT TITLE,IGNORE_SCHEDULING FROM institute_periods WHERE period_id={$days['PERIOD_ID']}"));
        $period_time = $period_time[1];
...
function VerifyVariableSchedule_Update($columns)
{
    ...
    $period_time = DBGet(DBQuery("SELECT IGNORE_SCHEDULING FROM institute_periods WHERE period_id={$per_id}"));
    $period_time = $period_time[1];
    $pre_ing = $period_time['IGNORE_SCHEDULING'];
    $min_start_dt_chk_qr = DBGet(DBQuery("SELECT min(start_date) as start_date from  staff_institute_relationship where staff_id in($all_teacher)"));
    $min_start_dt_chk_pr = DBGet(DBQuery("SELECT min(start_date) as start_date from  staff_institute_relationship where staff_id in($teacher)"));
...
function VerifyBlockedSchedule($columns, $course_period_id, $sec, $edit = false)
{
    ...
    if ($sec == 'cpv') {
        ...
        foreach ($cp_dates as $key => $cp) {
            ...
            $period_time = DBGet(DBQuery("SELECT IGNORE_SCHEDULING FROM institute_periods WHERE period_id={$period}"));
            $period_time = $period_time[1];
...
function VerifyStudentSchedule($course_RET, $student_id = '')
{
    ...
    if (count($course_RET) > 0) {
        $schedule_exist = DBGet(DBQuery("SELECT *  FROM `schedule` WHERE `syear` =" . $course_RET[1]['SYEAR'] . " AND `institute_id` =" . $course_RET[1]['INSTITUTE_ID'] . " AND `student_id` =" . $student_id . " AND `course_id` =" . $course_RET[1]['COURSE_ID'] . " AND `course_period_id` = " . $course_RET[1]['COURSE_PERIOD_ID'] . ' AND (END_DATE>="' . date('Y-m-d') . '" OR END_DATE IS NULL OR END_DATE="0000-00-00")'));
    ...
    if ($course_RET[1]['SCHEDULE_TYPE'] == 'FIXED') {
        ...
        // $period_days_append_sql = " AND course_period_id IN(SELECT course_period_id from course_period_var cpv,institute_periods sp WHERE cpv.period_id=sp.period_id AND ignore_scheduling IS NULL AND sp.start_time<='$end_time' AND '$start_time'<=sp.end_time AND (";
        $period_days_append_sql = " AND course_period_id IN(SELECT course_period_id from course_period_var cpv,institute_periods sp WHERE cpv.period_id=sp.period_id AND ignore_scheduling IS NULL AND sp.start_time >= '$start_time' AND sp.end_time <= '$end_time' AND (";
        ...
        $period_days_append_sql = " AND course_period_id IN(SELECT course_period_id from course_period_var cpv,institute_periods sp WHERE cpv.period_id=sp.period_id AND ignore_scheduling IS NULL AND (";
