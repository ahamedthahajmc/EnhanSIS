...
if (!$_REQUEST['marking_period_id'] && count($fy_RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' ORDER BY SORT_ORDER'))) == 1 && !$_REQUEST['ajax']) {
...
if (clean_param($_REQUEST['tables'], PARAM_NOTAGS) && ($_POST['tables'] || $_REQUEST['ajax']) && AllowEdit()) {
    ...
    foreach ($_REQUEST['tables'] as $id => $columns) {
        if ($table == 'institute_years') {
            $chk_tbl = 'institute_semesters';
            $nm = "semester";
            $date_sql = 'SELECT MIN(START_DATE) AS START_DATE,MAX(END_DATE) AS END_DATE FROM ' . $chk_tbl . ' WHERE YEAR_ID = \'' . $_REQUEST['marking_period_id'] . '\' AND INSTITUTE_ID =\'' .  UserInstitute() . '\' AND SYEAR = \'' .  UserSyear() . '\'';
            $dates =  DBGet(DBQuery($date_sql));
            $dates = $dates[1];
            $value = date('Y-m-d', strtotime($columns['START_DATE']));
            $prev_fy_sql = 'SELECT END_DATE FROM institute_years WHERE INSTITUTE_ID =\'' .  UserInstitute() . '\' AND SYEAR < \'' . UserSyear() . '\'';
            $prev_fy_dates = DBGet(DBQuery($prev_fy_sql));
            $prev_fy_dates = $prev_fy_dates[1];
        }
        if ($table == 'institute_semesters') {
            $chk_tbl = 'institute_years';
            $nm = "full year";
            $date_sql = 'SELECT START_DATE,END_DATE FROM ' . $chk_tbl . ' WHERE MARKING_PERIOD_ID = ' . $_REQUEST['year_id'] . ' AND INSTITUTE_ID =\'' .  UserInstitute() . '\' AND SYEAR = \'' .  UserSyear() . '\'';
            $dates =  DBGet(DBQuery($date_sql));
            $dates = $dates[1];
        }
        if ($table == 'institute_quarters') {
            $chk_tbl = 'institute_semesters';
            $nm = "semester";
            $date_sql = 'SELECT START_DATE,END_DATE FROM ' . $chk_tbl . ' WHERE MARKING_PERIOD_ID = ' . $_REQUEST['semester_id'] . ' AND INSTITUTE_ID =\'' .  UserInstitute() . '\' AND SYEAR = \'' .  UserSyear() . '\'';
            $dates =  DBGet(DBQuery($date_sql));
            $dates = $dates[1];
        }
        if ($table == 'institute_progress_periods') {
            $chk_tbl = 'institute_quarters';
            $nm = "quarter";
            $date_sql = 'SELECT START_DATE,END_DATE FROM ' . $chk_tbl . ' WHERE MARKING_PERIOD_ID = ' . $_REQUEST['quarter_id'] . ' AND INSTITUTE_ID =\'' .  UserInstitute() . '\' AND SYEAR = \'' .  UserSyear() . '\'';
            $dates =  DBGet(DBQuery($date_sql));
            $dates = $dates[1];
        }
        if ($id != 'new') {
                ...
                if ($error != true) {
                    ...
                    foreach ($columns as $column => $value) {
                        ...
                        if ($column == 'TITLE' && $columns['TITLE'] != '') {
                            $TITLE_COUNT =  DBGet(DBQuery('SELECT * FROM marking_periods WHERE upper(TITLE)=\'' . strtoupper(singleQuoteReplace('', '', $value)) . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'AND MARKING_PERIOD_ID NOT IN(\'' . $_REQUEST['marking_period_id'] . '\')'));
                                ...
                                if ($dates['START_DATE'] != '') {
                                    if (strtotime($dates['START_DATE']) >= strtotime($columns['START_DATE'])) {
                                        if (strtotime($columns['START_DATE']) <= strtotime($prev_fy_dates['END_DATE'])) {
                                            ...
                                            $cal_sql = 'SELECT CALENDAR_ID FROM institute_calendars WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                            $calender =  DBGet(DBQuery($cal_sql));
                                            $calender = $calender[1];
                                            $attendance_calendar = DBGet(DBQuery('SELECT MIN(INSTITUTE_DATE) as START_DATE,MAX(INSTITUTE_DATE) as END_DATE FROM attendance_calendar WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\''));
                                            ...
                                            if (strtotime($columns['START_DATE']) > strtotime($attendance_calendar[1]['START_DATE']) && $attendance_calendar[1]['START_DATE'] != '') {
                                                ...
                                                $stu_sql = 'SELECT COUNT(s.STUDENT_ID) AS TOTAL_REC FROM student_enrollment se,students s WHERE se.INSTITUTE_ID=\'' . UserInstitute() . '\' AND se.SYEAR=\'' . UserSyear() . '\' AND se.END_DATE IS NULL AND s.IS_DISABLE IS NULL';
                                                ...
                                                if ($students > 0 && $syear != UserSyear()) {
                                                    ...
                                                    $stf_sql = 'SELECT ssr.STAFF_ID FROM staff s,staff_institute_relationship ssr WHERE s.PROFILE_ID =\'2\' AND ssr.INSTITUTE_ID=\'' . UserInstitute() . '\' AND ssr.SYEAR=\'' . UserSyear() . '\'';
                                                    ...
                                                    if (is_countable($staffs) && count($staffs) > 0 && $syear != UserSyear()) {
                                                        ...
                                                        $subj_sql = 'SELECT SUBJECT_ID FROM course_subjects WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                        ...
                                                        if (is_countable($subjects) && count($subjects) > 0 && $syear != UserSyear()) {
                                                            ...
                                                            $att_codes_sql = 'SELECT ID FROM attendance_codes WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                            ...
                                                            if (is_countable($att_codes) && count($att_codes) > 0 && $syear != UserSyear()) {
                                                                ...
                                                                $sp_sql = 'SELECT PERIOD_ID FROM institute_periods WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                                $sp =  DBGet(DBQuery($sp_sql));
                                                                $sp = $sp[1];

                                                                if (is_countable($sp) && count($sp) > 0 && $syear != UserSyear()) {
                                                                    $err_msg = "" . _startDateCannotBeChangedBecauseTheFullYearWithInstitutePeriods . "";
                                                                    break 2;
                                                                } else {
                                                                    $fy_sql = 'SELECT SYEAR FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . $syear . '\'';
                                                                    ...
                                                                    if (is_countable($fy) && count($fy) > 0 && $syear != UserSyear()) {
                                                                        ...
                                                                        if ($value != '') {
                                                                            ...
                                                                            $fy_sql = 'UPDATE institute_years SET SYEAR=\'' . $syear . '\' WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                                            DBQuery($fy_sql);
                                                                            $sem_sql = 'UPDATE  institute_semesters SET SYEAR=\'' . $syear . '\' WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                                            DBQuery($sem_sql);
                                                                            $qtr_sql = 'UPDATE  institute_quarters SET SYEAR=\'' . $syear . '\' WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                                            DBQuery($qtr_sql);
                                                                            $progp_sql = 'UPDATE institute_progress_periods SET SYEAR=\'' . $syear . '\' WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'';
                                                                            DBQuery($progp_sql);
                                                                        }
                                                                    }
                                                                }
                                    ...
                                    if (strtotime($columns['START_DATE']) <= strtotime($prev_fy_dates['END_DATE'])) {
                                        ...
                                        if ($attendance_calendar[1]['START_DATE'] != '' && strtotime($columns['START_DATE']) > strtotime($attendance_calendar[1]['START_DATE'])) {
                                            ...
                                            $stu_sql = 'SELECT s.STUDENT_ID FROM student_enrollment se,students s WHERE se.INSTITUTE_ID=\'' . UserInstitute() . '\' AND se.SYEAR=\'' . UserSyear() . '\' AND se.END_DATE IS NULL AND s.IS_DISABLE IS NULL';
                                            $students =  DBGet(DBQuery($stu_sql));
                                            $students = $students[1];
                                            ...
                                            if (is_countable($students) && count($students) > 0 && $syear != UserSyear()) {
                                                ...
                                                $stf_sql = 'SELECT ssr.STAFF_ID FROM staff s,staff_institute_relationship ssr WHERE s.PROFILE_ID in (\'0\',\'1\',\'2\') AND ssr.INSTITUTE_ID=\'' . UserInstitute() . '\' AND ssr.SYEAR=\'' . UserSyear() . '\'';
                                                $staffs =  DBGet(DBQuery($stf_sql));
                                                $staffs = $staffs[1];
            ...
            $fields = 'MARKING_PERIOD_ID,SYEAR,INSTITUTE_ID,';
            $values = '\'' . $id_RET[1]['ID'] . '\',\'' . UserSyear() . '\',\'' . UserInstitute() . '\',';
            ...
            foreach ($columns as $column => $value) {
                ...
                if ($value) {
                    if ($column == 'TITLE' && $columns['TITLE'] != '') {
                        $TITLE_COUNT =  DBGet(DBQuery('SELECT * FROM marking_periods WHERE upper(TITLE)=\'' . strtoupper(singleQuoteReplace('', '', $value)) . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\''));
                        if (is_countable($TITLE_COUNT) && count($TITLE_COUNT) > 0) {
        ...
        $dates_RET = DBGet(DBQuery(
            'SELECT MARKING_PERIOD_ID FROM ' . $table . ' WHERE (true=false'
                ...
                . ') AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'' . (($id != 'new') ? ' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' AND MARKING_PERIOD_ID!=\'' . $id . '\'' : '')
        ));
        $posting_RET = DBGet(DBQuery(
            'SELECT MARKING_PERIOD_ID FROM ' . $table . ' WHERE (true=false'
                ...
                . ') AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'' . (($id != 'new') ? ' AND MARKING_PERIOD_ID!=\'' . $id . '\'' : '')
        ));
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'delete') {
    ...
    switch ($table) {
        case 'institute_years':
            ...
            $extra[] = 'DELETE FROM institute_progress_periods WHERE QUARTER_ID IN (SELECT MARKING_PERIOD_ID FROM institute_quarters WHERE SEMESTER_ID IN (SELECT MARKING_PERIOD_ID FROM institute_semesters WHERE YEAR_ID=\'' . $year_id . '\'))';
            $extra[] = 'DELETE FROM institute_quarters WHERE SEMESTER_ID IN (SELECT MARKING_PERIOD_ID FROM institute_semesters WHERE YEAR_ID=\'' . $year_id . '\')';
            $extra[] = 'DELETE FROM institute_semesters WHERE YEAR_ID=\'' . $year_id . '\'';
            break;
        ...
        case 'institute_semesters':
            ...
            $parent_table = 'institute_years';
            $sems_id = paramlib_validation($column = 'MARKING_PERIOD_ID', $_REQUEST['marking_period_id']);
            $extra[] = 'DELETE FROM institute_progress_periods WHERE QUARTER_ID IN (SELECT MARKING_PERIOD_ID FROM institute_quarters WHERE SEMESTER_ID=\'' . $sems_id . '\')';
            $extra[] = 'DELETE FROM institute_quarters WHERE SEMESTER_ID=\'' . $sems_id . '\'';
            break;
        ...
        case 'institute_quarters':
            ...
            $parent_table = 'institute_semesters';
            $qrt_id = paramlib_validation($column = 'MARKING_PERIOD_ID', $_REQUEST['marking_period_id']);
            $extra[] = 'DELETE FROM institute_progress_periods WHERE QUARTER_ID=\'' . $qrt_id . '\'';
            break;
        ...
        case 'institute_progress_periods':
            ...
            $parent_table = 'institute_quarters';
            break;
    }
    ...
    if ($has_assigned > 0) {
        ...
        if ($table == 'institute_quarters')
            $check_other_assocs = DBGet(DBQuery('SELECT COUNT(1) as REC_EX FROM institute_progress_periods WHERE QUARTER_ID=' . $_REQUEST['marking_period_id']));
        else
...
if (!$_REQUEST['modfunc']) {
    ...
    if (clean_param($_REQUEST['marking_period_id'], PARAM_ALPHANUM)) {
        ...
        echo '<div>';
        if (clean_param($_REQUEST['marking_period_id'], PARAM_ALPHANUM) == 'new') {
            echo AllowEdit() ? '<hr/><div class="text-right"><INPUT type=submit value=' . _save . ' id="setupMPBtn" class="btn btn-primary" onclick="return formcheck_institute_setup_marking(this);"></div>' : '';
    ...
    // FY
    $sql = 'SELECT MARKING_PERIOD_ID,TITLE FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' ORDER BY SORT_ORDER';
    $QI = DBQuery($sql);
    $fy_RET = DBGet($QI);
    ...
    if (($_REQUEST['mp_term'] == 'FY' && $_REQUEST['marking_period_id'] != 'new') || $_REQUEST['mp_term'] == 'SEM' || $_REQUEST['mp_term'] == 'QTR' || $_REQUEST['mp_term'] == 'PRO') {
        $sql = 'SELECT MARKING_PERIOD_ID,TITLE FROM institute_semesters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' AND YEAR_ID=\'' . $_REQUEST['year_id'] . '\' ORDER BY SORT_ORDER';
        $QI = DBQuery($sql);
        $sem_RET = DBGet($QI);
        ...
        $sem_edate = DBGet(DBQuery('SELECT MAX(END_DATE) AS END_DATE,MIN(START_DATE) as START_DATE FROM institute_semesters WHERE INSTITUTE_ID=' . UserInstitute() . ' AND SYEAR=' . UserSyear()));
        $sem_sdate = $sem_edate[1]['START_DATE'];
        $sem_edate = $sem_edate[1]['END_DATE'];
        $fy_edate = DBGet(DBQuery('SELECT END_DATE,START_DATE FROM institute_years WHERE INSTITUTE_ID=' . UserInstitute() . ' AND SYEAR=' . UserSyear()));
        ...
        if (($_REQUEST['mp_term'] == 'SEM' && $_REQUEST['marking_period_id'] != 'new') || $_REQUEST['mp_term'] == 'QTR' || $_REQUEST['mp_term'] == 'PRO') {
            $sql = 'SELECT MARKING_PERIOD_ID,TITLE FROM institute_quarters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' AND SEMESTER_ID=\'' . $_REQUEST['semester_id'] . '\' ORDER BY SORT_ORDER';
            $QI = DBQuery($sql);
            $qtr_RET = DBGet($QI);
            ...
            $sem_edate = DBGet(DBQuery('SELECT END_DATE,START_DATE FROM institute_semesters WHERE MARKING_PERIOD_ID=' . $_REQUEST['semester_id']));
            $sem_sdate = $sem_edate[1]['START_DATE'];
            $sem_edate = $sem_edate[1]['END_DATE'];
            $qtr_edate = DBGet(DBQuery('SELECT MAX(END_DATE) AS END_DATE,MIN(START_DATE) as START_DATE FROM institute_quarters WHERE SEMESTER_ID=' . $_REQUEST['semester_id']));
            ...
            // PROGRESS PERIODS
            if (($_REQUEST['mp_term'] == 'QTR' && $_REQUEST['marking_period_id'] != 'new') || $_REQUEST['mp_term'] == 'PRO') {
                $sql = 'SELECT MARKING_PERIOD_ID,TITLE FROM institute_progress_periods WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' AND QUARTER_ID=\'' . $_REQUEST['quarter_id'] . '\' ORDER BY SORT_ORDER';
                $QI = DBQuery($sql);
                $pro_RET = DBGet($QI);
                ...
                $sql_mp_id = 'SELECT MARKING_PERIOD_ID,TITLE FROM institute_progress_periods';
            }
        }
    }
