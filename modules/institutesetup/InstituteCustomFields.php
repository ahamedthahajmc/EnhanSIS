...
echo '<input id="customFieldModule" type="hidden" value="institute">';
...
if (clean_param($_REQUEST['tables'], PARAM_NOTAGS) && ($_POST['tables'] || $_REQUEST['ajax'])) {
    ...
    if($_REQUEST['SYSTEM_WIDE']=='Y')
        $_REQUEST['tables'][$_REQUEST['custom']]['INSTITUTE_ID']=0;
    else
        $_REQUEST['tables'][$_REQUEST['custom']]['INSTITUTE_ID']= UserInstitute();
    ...
    foreach ($_REQUEST['tables'] as $id => $columns) {
        ...
        if ($id != 'new') {
            ...
            $go = true;
            if ($table == 'institute_custom_fields')
                $custom_field_id = $id;

            // print_r($_REQUEST);exit;
            if ($custom_field_id) {

                $chk_cus_data = DBGet(DBQuery('SELECT * from institutes WHERE SYEAR=' . UserSyear() . ' AND id=' . UserInstitute()));
                ...
                $custom_update = DBGet(DBQuery("SELECT TYPE,REQUIRED,DEFAULT_SELECTION,HIDE FROM institute_custom_fields WHERE ID=$custom_field_id"));
                
                $custom_update = $custom_update[1];
                switch ($custom_update['TYPE']) {
                    case 'radio':
                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id VARCHAR(1) ";
                        break;

                    case 'text':
                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id VARCHAR(255)";
                        break;

                    case 'select':
                    case 'autos':
                    case 'edits':
                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id VARCHAR(100)";
                        break;

                    case 'codeds':
                        $colLength = typeLength($custom_update['DEFAULT_SELECTION'], 15);

                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id VARCHAR(".$colLength.")";
                        break;
                    ...
                    case 'numeric':
                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id NUMERIC(20,2)";
                    ...
                    case 'date':
                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id  DATE";
                    ...
                    case 'textarea':
                        $Sql_modify_column = "ALTER TABLE institutes MODIFY CUSTOM_$id LONGTEXT";
                        // $not_default = true;
                        break;
                }
                ...
                if ($custom_update['DEFAULT_SELECTION'] && $not_default == false) {
                    ...
                    $existing_column_updt = 'UPDATE `institutes` SET CUSTOM_'.$id.' = \''.$custom_update['DEFAULT_SELECTION'].'\' WHERE (CUSTOM_'.$id.' IS NULL OR CUSTOM_'.$id.' = "")';
                }
                ...
                if ($flag == 0) {
                    ...
                    if($sort_oder_to_change!=0){
                        DBQuery('UPDATE institute_custom_fields SET SORT_ORDER=\''.$sort_oder_to_change.'\' WHERE ID='.$custom_field_id);
                    }
                    else{
            ...
            if ($table == 'institute_custom_fields') {
                ...
                // $id = DBGet(DBQuery("SHOW TABLE STATUS LIKE 'institute_custom_fields'"));
                ...
                switch ($columns['TYPE']) {
                    case 'radio':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id VARCHAR(1) ";
                        break;

                    case 'text':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id VARCHAR(255)";
                        break;

                    case 'select':
                    case 'autos':
                    case 'edits':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id VARCHAR(100)";
                        break;

                    case 'codeds':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id VARCHAR(15)";
                        break;
                    ...
                    case 'numeric':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id NUMERIC(20,2)";
                        if (!is_numeric($columns['DEFAULT_SELECTION'])) {
                            $not_default = true;
                    ...
                    case 'date':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id  DATE";
                        if (preg_match("/^[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}$/", $columns['DEFAULT_SELECTION']) === 0) {
                            $not_default = true;
                            $columns['DEFAULT_SELECTION'] = '';
                        }
                    ...
                    case 'textarea':
                        $Sql_add_column = "ALTER TABLE institutes ADD CUSTOM_$id LONGTEXT";
                        $not_default = true;
                        break;
                }
                if ($columns['REQUIRED']) {
    ...
    // echo '<script>window.location.href="Modules.php?modname=institutesetup/InstituteCustomFields.php"</script>';
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'delete') {
    if (clean_param($_REQUEST['id'], PARAM_INT)) {
        ...
        $chk_sch_cus = DBGet(DBQuery('SELECT COUNT(*) AS REC_EX FROM institutes WHERE (' . $sc_cus_fld_column . '<>\'\' AND  ' . $sc_cus_fld_column . ' is NOT NULL)'));

        if ($chk_sch_cus[1]['REC_EX'] == 0) {
            if (DeletePromptCommon('institute field')) {
                $id = clean_param($_REQUEST['id'], PARAM_INT);
                DBQuery('DELETE FROM institute_custom_fields WHERE ID=\'' . $id . '\'');
                DBQuery('ALTER TABLE institutes DROP COLUMN CUSTOM_' . $id . '');
                $_REQUEST['modfunc'] = '';
                unset($_REQUEST['id']);
            }
        } else {
...
if ($_REQUEST['id'] && $_REQUEST['id'] != 'new') {
    $sql = "SELECT INSTITUTE_ID,CATEGORY_ID,TITLE,TYPE,SELECT_OPTIONS,DEFAULT_SELECTION,SORT_ORDER,REQUIRED,REQUIRED,HIDE FROM institute_custom_fields WHERE ID='".$_REQUEST['id']."' ";
    $RET = DBGet(DBQuery($sql));
    $RET = $RET[1];
    $title = $RET['TITLE'];
} elseif ($_REQUEST['id'] == 'new') {
    $title = _newInstituteField;
}
...
if ($_REQUEST['id'] && !$_REQUEST['modfunc']) {
    ...
    if ($_REQUEST['id'] != 'new')
        ...
        $action .= "&table=institute_custom_fields";    
    ...
    $header .= '<a href="Modules.php?modname=institutesetup/InstituteCustomFields.php"><i class="icon-square-left m-r-5"></i> ' . _backToInstituteCustomFields . '</a>';
    $header .= '</li>';
    $header .= '</ul>';
    $header .= '</div>';

    $header .= '<div class="panel-body">';
    $header .= '<input type=hidden name=tables[' . $_REQUEST['id'] . '][INSTITUTE_ID] value=' . UserInstitute() . '>';
    ...
    $header .= '</div>';
    $header .= '<div class="panel-footer"><div class="heading-elements">' . SubmitButton(_save, '', 'id="setupInstituteFieldsBtn" class="btn btn-primary heading-btn pull-right ml-10" onclick="formcheck_institutefields(this);"') . $delete_button . '</div></div>';
    $header .= '</div>'; //.panel
    ...
    $columns = array('TITLE' =>_instituteFields,'SORT_ORDER' =>_sortOrder, 'TYPE' =>_fieldType);
    $link = array();
    $arr = array(_instituteName,
     _address,
     _city,
     _state,
     _zipPostalCode,
    ...
    $RET = DBGet(DBQuery("SELECT * FROM institute_custom_fields WHERE INSTITUTE_ID IN (" . UserInstitute() .",0) ORDER BY SORT_ORDER"));
    ...
    ListOutput($dd, $columns, _instituteField, _instituteFields, $link, array(), $LO_options);
    echo '</div>'; //.panel
