...
function PrepareDate($date = '', $title = '', $allow_na = true, $options = '', $cal_img = 'Y') {
    ...
    $return .= '<div id="date_div_'.$_HaniIMS['PrepareDate'].'" class="fake_datepicker" onclick="$(\'#original_date_' . $_HaniIMS['PrepareDate'] . '\').show(); $(\'#date_div_'.$_HaniIMS['PrepareDate'].'\').hide();">';
    ...
    $return .= '<div class="input-group '.(($options['view']=='month')?'datepicker-group-month':'datepicker-group').'" style="display: none;" id="original_date_' . $_HaniIMS['PrepareDate'] . '" value="' . date('Y-m-d', strtotime($date)) . '">';
    $return .= '<span class="input-group-addon"><i class="icon-calendar22"></i></span>';
    $return .= '<input id="date_' . $_HaniIMS['PrepareDate'] . '" placeholder="" value="' .(($options['view']=='month')?date('m/Y', strtotime($date)):date('Y-m-d', strtotime($date))). '" class="form-control daterange-single" type="text">';
    $return .= '</div>';

    // MONTH  ---------------
    if ($options['M']) {
//		$return .= "<SELECT NAME=month".$title." class=\"form-control\" id=monthSelect".$_HaniIMS['PrepareDate']." $extraM>";
//		//  -------------------------------------------------------------------------- //
//		
//		if($month == 'JAN')
//			$month = 1;
...
//		
//		$return .= "</SELECT>";
        $return .= '<input value="' . date('m', strtotime($date)) . '" id="monthSelect_date_' . $_HaniIMS['PrepareDate'] . '" name="month' . $title . '" type="hidden">';
    }

    // DAY  ---------------
    if ($options['D']) {
//        $return .= "<SELECT NAME=day" . $title . " id=daySelect" . $_HaniIMS['PrepareDate'] . " class=\"form-control\" $extraD>";
//        if ($allow_na) {
//            if ($day == '00') {
...
//        $return .="</SELECT></div>";
        ...
        $return .= '<input value="' . date('d', strtotime($date)) . '" id="daySelect_date_' . $_HaniIMS['PrepareDate'] . '" name="day' . $title . '" type="hidden">';
    }
...
//        }
//
//        $return .="<div class=\"form-group\"><SELECT NAME=year" . $title . " id=yearSelect" . $_HaniIMS['PrepareDate'] . " class=\"form-control\" $extraY>";
//        if ($allow_na) {
//            if ($year == '00') {
//                $return .= "<OPTION value=\"\" SELECTED>N/A";
//            } else {
...
//        $return .="</SELECT></div>";
        ...
        $return .= '<input value="' . date('Y', strtotime($date)) . '" id="yearSelect_date_' . $_HaniIMS['PrepareDate'] . '" name="year' . $title . '" type="hidden">';
    }

//    if ($options['C']) {
//        if ($cal_img != 'N') {
//            $return .= '<span style="padding-top:4px"><img src="assets/calendar.gif" id="trigger' . $_HaniIMS['PrepareDate'] . '" style="cursor: pointer;" onmouseover=this.style.background=""; onmouseout=this.style.background=""; onClick=' . "MakeDate('" . $_HaniIMS['PrepareDate'] . "',this);" . ' /></span>';
//        }
//    }
...
function PrepareDate_for_EndInput($date = '', $title = '', $allow_na = true, $options = '') {
    ...
    // MONTH  ---------------
    if ($options['M']) {
        $return .= "<div class=\"form-group\"><SELECT NAME=month" . $title . " id=monthSelect" . $_HaniIMS['PrepareDate'] . " class=\"form-control\" $extraM>";
        //  -------------------------------------------------------------------------- //
    ...
    // DAY  ---------------
    if ($options['D']) {
        $return .="<div class=\"form-group\"><SELECT NAME=day" . $title . " id=daySelect" . $_HaniIMS['PrepareDate'] . " class=\"form-control\" $extraD>";
        if ($allow_na) {
            if ($day == '00') {
                $return .= "<OPTION value=\"\" SELECTED>N/A";
            } else {
    ...
    if ($options['Y']) {
        ...
        $return .="<div class=\"form-group\"><SELECT NAME=year" . $title . " id=yearSelect" . $_HaniIMS['PrepareDate'] . " SIZE=1 $extraY>";
        if ($allow_na) {
            if ($year == '00') {
                $return .= "<OPTION value=\"\" SELECTED>N/A";
            } else {
