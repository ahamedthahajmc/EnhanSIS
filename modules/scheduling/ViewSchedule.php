...
if (UserStudentID()) {
    ...
    switch ($_REQUEST['view_mode']) {
        case 'day_view':
            if ((User('PROFILE_ID') == 3 || User('PROFILE_ID') == 4) && $date == '')
                $date = date('Y-m-d');
            $mp_sql = 'SELECT MARKING_PERIOD_ID,START_DATE,END_DATE FROM marking_periods WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND \'' . date('Y-m-d', strtotime($date)) . '\' BETWEEN START_DATE AND END_DATE';
            $mp_data = DBGet(DBQuery($mp_sql));
            if (count($mp_data) == 0) {
                $mp_sql = 'SELECT MARKING_PERIOD_ID,START_DATE,END_DATE FROM marking_periods WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\'';
                $mp_data = DBGet(DBQuery($mp_sql));
            }
            ...
            FROM schedule s,courses c,course_periods cp,institute_periods sp,course_period_var cpv,rooms r, attendance_calendar ac
            ...
            AND s.INSTITUTE_ID = sp.INSTITUTE_ID 
            AND s.SYEAR = c.SYEAR 
            ...
            AND s.INSTITUTE_ID = \'' . UserInstitute() . '\' 
            ...
            $sql .= 'AND ac.INSTITUTE_ID = s.INSTITUTE_ID AND ac.INSTITUTE_DATE = \'' . date('Y-m-d', strtotime($date)) . '\' ';
        ...
        case 'week_view':
            $cal_RET = DBGet(DBQuery('SELECT START_DATE,END_DATE FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\''));
            ...
            $QI = ($sql);
            $wk_schedule_RET = DBGet(DBQuery('SELECT sp.PERIOD_ID,CONCAT(sp.START_TIME,\'' . ' - ' . '\',sp.END_TIME) AS TIME_PERIOD,sp.TITLE FROM institute_periods sp WHERE sp.SYEAR=\'' . UserSyear() . '\' AND sp.INSTITUTE_ID = \'' . UserInstitute() . '\' ORDER BY sp.SORT_ORDER'), array('TIME_PERIOD' => '_makeTimePeriod'));

            $mp_start_date = DBGET(DBQuery('SELECT start_date FROM marking_periods WHERE MARKING_PERIOD_ID = "' . $_REQUEST['marking_period_id'] . '"'));


            $sql_week = 'SELECT acc.INSTITUTE_DATE,cp.TITLE,cp.COURSE_PERIOD_ID,cp.TEACHER_ID,cpv.PERIOD_ID
            FROM attendance_calendar acc
            INNER JOIN marking_periods mp ON mp.SYEAR=acc.SYEAR AND mp.INSTITUTE_ID=acc.INSTITUTE_ID
            AND acc.INSTITUTE_DATE BETWEEN mp.START_DATE AND mp.END_DATE
            ...
            INNER JOIN institute_periods sp ON sp.SYEAR=acc.SYEAR AND sp.INSTITUTE_ID=acc.INSTITUTE_ID AND sp.PERIOD_ID=cpv.PERIOD_ID
                                                                    INNER JOIN schedule sch ON sch.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND sch.START_DATE<=acc.INSTITUTE_DATE AND (sch.END_DATE IS NULL OR sch.END_DATE>=acc.INSTITUTE_DATE) AND ';

            if (User('PROFILE_ID') != 2) {

                $sql_week .= 'acc.INSTITUTE_DATE BETWEEN \'' . $week_start . '\' AND \'' . $week_end . '\'
                ...
                if ($_REQUEST['include_inactive'] != 'Y') {
                    ...
                    $sql_week .= 'acc.INSTITUTE_DATE BETWEEN \'' . $mp_start_date[1]['START_DATE'] . '\' AND \'' . $week_end . '\'
                    AND sch.STUDENT_ID=\'' . UserStudentID() . '\'';
                }
            }

            // echo $sql_week;
            $week_RET = DBGet(DBQuery($sql_week), array(), array('INSTITUTE_DATE', 'PERIOD_ID'));
            ...
            if (User('PROFILE_ID') != 2) {
                ...
                $sql_custom_schedule .= 'AND (\'' . date('Y-m-d', strtotime($date)) . '\' BETWEEN s.START_DATE AND s.END_DATE OR (s.END_DATE IS NULL AND s.START_DATE<=\'' . date('Y-m-d', strtotime($date)) . '\')) AND s.STUDENT_ID=' . UserStudentID() . ' AND s.INSTITUTE_ID=' . UserInstitute();
            } else {
                ...
                foreach ($wk_schedule_RET as $course) {
                    ...
                    for ($j = $today; $j <= $today + $one_day * 6; $j = $j + $one_day) {
                        ...
                        if (is_array($week_RET[date('Y-m-d', $j)][$course['PERIOD_ID']][1]) && in_array(date('Y-m-d', $j), $week_RET[date('Y-m-d', $j)][$course['PERIOD_ID']][1])) {
                            ...
                            $day = date('l', strtotime($week_RET[date('Y-m-d', $j)][$course['PERIOD_ID']][1]['INSTITUTE_DATE']));
        ...
        case 'month_view':
            ...
            $calendar_RET = DBGet(DBQuery('SELECT INSTITUTE_DATE,MINUTES,BLOCK FROM attendance_calendar WHERE INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', $time) . '\' AND \'' . date('Y-m-d', mktime(0, 0, 0, $month, $last, $year)) . '\' AND SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\''), array(), array('INSTITUTE_DATE'));
			...
			FROM schedule s,courses c,course_periods cp,course_period_var cpv,institute_periods sp,rooms r
			WHERE
				...
				AND s.INSTITUTE_ID = sp.INSTITUTE_ID AND s.SYEAR = c.SYEAR AND sp.PERIOD_ID = cpv.PERIOD_ID
                                                                        AND POSITION(\'' . $day . '\' IN cpv.days)>0
				AND s.STUDENT_ID=\'' . UserStudentID() . '\'
                AND s.SYEAR=\'' . UserSyear() . '\' AND s.INSTITUTE_ID = \'' . UserInstitute() . '\' AND (cpv.COURSE_PERIOD_DATE=\'' . date('Y-m-d', strtotime($date)) . '\' OR cpv.COURSE_PERIOD_DATE IS NULL) ';
    ...
    if ($_REQUEST['modfunc'] == 'detail') {
        ...
        $sql = 'SELECT s.ID AS SCHEDULE_ID,
				...
				c.TITLE ,cp.COURSE_PERIOD_ID AS PERIOD_PULLDOWN,
				s.STUDENT_ID,r.TITLE AS ROOM,cpv.DAYS,SCHEDULER_LOCK
			FROM schedule s,courses c,course_periods cp,institute_periods sp,course_period_var cpv,rooms r
			WHERE
				s.COURSE_ID = c.COURSE_ID AND s.COURSE_ID = cp.COURSE_ID
                                AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID
				AND s.COURSE_PERIOD_ID = cp.COURSE_PERIOD_ID
    ...
    if ($schedule_RET && $_REQUEST['view_mode'] == 'day_view') {
        ...
        echo ProgramLinkforExport('scheduling/PrintSchedules.php', '<b><i class="icon-printer4"></i></b>' . _printSchedule . '', '&modfunc=save&st_arr[]=' . UserStudentID() . '&mp_id=' . $mp_id . '&include_inactive=' . $_REQUEST['include_inactive'] . '&date1=' . $date1 . '&HaniIMS_PDF=true', 'target="_blank" class="btn btn-success btn-labeled"') . '</div>';
        echo '</div>'; //.panel-body
...
function _makePeriodSelect($course_period_id, $column = '')
{
    ...
    $sql = 'SELECT cp.COURSE_PERIOD_ID,cp.PARENT_ID,cp.TITLE,cp.MARKING_PERIOD_ID,COALESCE(cp.TOTAL_SEATS-cp.FILLED_SEATS,0) AS AVAILABLE_SEATS FROM course_periods cp,institute_periods sp,course_period_var cpv WHERE sp.PERIOD_ID=cpv.PERIOD_ID AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cp.COURSE_ID=\'' . $THIS_RET['COURSE_ID'] . '\' ORDER BY sp.SORT_ORDER';
    $QI = DBQuery($sql);
    $orders_RET = DBGet($QI);
...
$res = DBGet(DBQuery('SELECT CONCAT(students.LAST_NAME,\', \',coalesce(students.COMMON_NAME, students.FIRST_NAME)) AS FULL_NAME,
    ...
    institute_gradelevels.title
    FROM
        students
    INNER JOIN student_enrollment ON students.student_id = student_enrollment.student_id
    INNER JOIN institute_gradelevels ON student_enrollment.grade_id=institute_gradelevels.id
    ...
    echo "<tr><td style=\"font-size:15px; font-weight:bold;\">" . GetInstitute(UserInstitute()) . "<br><div style=\"font-size:12px;\">" . _studentSchedulesReport . "</div></td><td style=\"float:right;text-align: right;\">" . ProperDate(DBDate()) . "<br />" . _poweredByhani . "</td></tr><tr><td colspan=2><hr></td></tr></table>";
