...
if ((($_REQUEST['profiles'] && ($_POST['profiles']  || $_REQUEST['ajax'])) || ($_REQUEST['values'] && ($_POST['values'] || $_REQUEST['ajax']))) && AllowEdit()) {
	$notes_RET = DBGet(DBQuery('SELECT ID FROM portal_notes WHERE (INSTITUTE_ID=\'' . UserInstitute() . '\' OR PUBLISHED_PROFILES LIKE \'%all%\') AND SYEAR=\'' . UserSyear() . '\''));
	...
	foreach ($_REQUEST['values'] as $id => $columns) {
		if (!(isset($columns['TITLE']) && trim($columns['TITLE']) == '')) {
			if ($id != 'new') {
				...
				$syear_RET = DBGet(DBQuery('SELECT START_DATE,END_DATE FROm institute_years WHERE INSTITUTE_ID=' . UserInstitute() . ' AND SYEAR=' . UserSyear()));
				...
				elseif (strtotime($columns['START_DATE']) > strtotime($columns['END_DATE'])) {
					...
					$fields = 'INSTITUTE_ID,SYEAR,last_updated,PUBLISHED_USER,';
					if ($allinstitute == 'Y')
						$values = 'NULL,\'' . UserSyear() . '\',CURRENT_TIMESTAMP,\'' . User('STAFF_ID') . '\',';
					else
						$values = UserInstitute() . ',\'' . UserSyear() . '\',CURRENT_TIMESTAMP,\'' . User('STAFF_ID') . '\',';
...
if ($_REQUEST['modfunc'] != 'remove') {
	...
	echo '<div class="panel-footer"><div class="col-md-12 text-right">' . SubmitButton(_save, '', 'id="savePortalBtn" class="btn btn-primary" onclick="formcheck_institute_setup_portalnotes(this);"') . '</div></div>';
	echo '</div></FORM>';
}
...
function _makePublishing($value, $name)
{
	...
	foreach (array('all' => _allInstitute, 'admin' => _administratorWCustom, 'teacher' => _teacherWCustom, 'parent' => _parentWCustom) as $profile_id => $profile)
