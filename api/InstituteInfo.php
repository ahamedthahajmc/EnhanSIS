...
{	global $_HaniIMS;
    $institute=sqlSecurityFilter($institute);
...
if(count($validate) > 0)
{
//    $institute_id=$_REQUEST['sch_id'];
    $institute_info=DBGet(DBQuery('SELECT * FROM institutes'));
    ...
    if($format == 'json') 
    {
        ...
        {
            $i=1;
            foreach($institute_info as $key=>$value)
            {
                foreach($value as $k=>$val)
                {
                    if($k!='LAST_UPDATED')
                    {
                        ...
                        {
                            if($k=='ID')
                                $data['data'][$i]['INSTITUTE_ID']=$val;
                            else if($k=='SYEAR')
                                $data['data'][$i]['INSTITUTE_YEAR']=$val;
                             else if($k=='TITLE')
                                $data['data'][$i]['INSTITUTE_NAME']=$val;
    ...
    if($format == 'xml')
    {
        ...
        if(count($institute_info)> 0)
        {
            echo '<data>';
            $i=1;
            foreach($institute_info as $key=>$val)
            {
                echo'<INSTITUTE_'.$i.'>';
                foreach ($val as $vkey => $value) {
                    if($vkey!='LAST_UPDATED')
                    {
                        ...
                        {
                            if($vkey=='ID')
                                 echo '<INSTITUTE_ID>'.htmlentities($value).'</INSTITUTE_ID>';
                            else if($vkey=='SYEAR')
                                 echo '<INSTITUTE_YEAR>'.htmlentities($value).'</INSTITUTE_YEAR>';
                            else if($vkey=='TITLE')
                                 echo '<INSTITUTE_NAME>'.htmlentities($value).'</INSTITUTE_NAME>';
