...
if ($_REQUEST['modfunc'] == 'save' && $_REQUEST['honor_roll']) {
    if (is_countable($_REQUEST['st_arr']) && count($_REQUEST['st_arr'])) {
        ...
        if ($_REQUEST['honor_roll'] != 986) {
            ...
            $scale = $INSTITUTE_RET[1]['REPORTING_GP_SCALE'];
            $honor = DBGet(DBQuery('SELECT VALUE  FROM honor_roll WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' ORDER BY VALUE DESC'));
            ...
            $institute_info_RET = DBGet(DBQuery('SELECT TITLE,PRINCIPAL FROM institutes WHERE ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\''));
            $extra['SELECT'] = ',coalesce(s.COMMON_NAME,s.FIRST_NAME) AS NICK_NAME';
            $extra['SELECT'] .= ',(SELECT SORT_ORDER FROM institute_gradelevels WHERE ID=ssm.GRADE_ID) AS SORT_ORDER';
            $extra['FROM'] .= ',student_report_card_grades srg';
            if ($_REQUEST['w_course_period_id']) {
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE  hr.INSTITUTE_ID=' . UserInstitute() . ' AND hr.SYEAR=' . UserSyear() . ' AND  hr.VALUE=(SELECT if((ROUND(AVG(srcg.grade_percent))>=' . $honor_gpa1 . ' and ROUND(AVG(srcg.grade_percent))<' . $honor_gpa2 . '),' . $honor_gpa1 . ',"")  FROM
                ...
                $extra['SELECT'] .= ',(SELECT CONCAT(st.LAST_NAME,", ",coalesce(st.FIRST_NAME)) FROM staff st,course_periods cp,course_period_var cpv,institute_periods p,schedule ss WHERE st.STAFF_ID=cp.TEACHER_ID AND cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND  cp.COURSE_PERIOD_ID=\'' . $_REQUEST['w_course_period_id'] . '\' AND ss.STUDENT_ID=s.STUDENT_ID AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date('Y-m-d', strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS TEACHER';
                $extra['SELECT'] .= ',(SELECT cpv.ROOM_ID AS ROOM FROM course_periods cp,course_period_var cpv,institute_periods p,schedule ss WHERE cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND  cp.COURSE_PERIOD_ID=\'' . $_REQUEST['w_course_period_id'] . '\' AND ss.STUDENT_ID=s.STUDENT_ID AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date("Y-m-d", strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS ROOM';
            } else {
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE  hr.VALUE=(SELECT if((ROUND(AVG(srcg.grade_percent))>=' . $honor_gpa1 . ' and ROUND(AVG(srcg.grade_percent))<' . $honor_gpa2 . '),' . $honor_gpa1 . ',"")  FROM
                                                   ...
                                                   and srcg.`STUDENT_ID`=ssm.STUDENT_ID)  AND hr.INSTITUTE_ID=' . UserInstitute() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                $extra['WHERE'] .= 'AND ((SELECT ROUND(AVG(srcg.grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y"
                                                   and srcg.`STUDENT_ID`=ssm.STUDENT_ID)>=' . $honor_gpa1 . ' ) AND ((SELECT ROUND(AVG(grade_percent)) FROM
                                                   `student_report_card_grades` srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y"
                                                   and srcg.`STUDENT_ID`=ssm.STUDENT_ID)<' . $honor_gpa2 . ' )  GROUP BY s.STUDENT_ID';
                $extra['SELECT'] .= ',(SELECT CONCAT(st.LAST_NAME,", ",coalesce(st.FIRST_NAME)) FROM staff st,course_periods cp,course_period_var cpv,institute_periods p,schedule ss WHERE st.STAFF_ID=cp.TEACHER_ID AND cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.STUDENT_ID=s.STUDENT_ID AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date('Y-m-d', strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS TEACHER';
                $extra['SELECT'] .= ',(SELECT cpv.ROOM_ID AS ROOM FROM course_periods cp,course_period_var cpv,institute_periods p,schedule ss WHERE cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE="Y" AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.STUDENT_ID=s.STUDENT_ID AND ss.SYEAR=' . UserSyear() . ' AND ss.MARKING_PERIOD_ID = ' . UserMp() . ' AND (ss.START_DATE<=' . date('Y-m-d', strtotime(DBDate())) . ' AND (ss.END_DATE>=' . date("Y-m-d", strtotime(DBDate())) . ' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS ROOM';
            }
            ...
            if ($_REQUEST['w_course_period_id']) {
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                ...
                                                                FROM `student_report_card_grades`srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll="Y" and `STUDENT_ID`=ssm.STUDENT_ID)  AND hr.INSTITUTE_ID=' . UserInstitute() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                ...
                $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                ...
                                                                and srcg.STUDENT_ID=ssm.STUDENT_ID) order by hr.value asc limit 1),(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE<=(SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y"
                                                                and srcg.STUDENT_ID=ssm.STUDENT_ID) order by hr.value desc limit 1))
                                                                FROM `student_report_card_grades`srcg,course_periods cpp  WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y" and `STUDENT_ID`=ssm.STUDENT_ID)  AND hr.INSTITUTE_ID=' . UserInstitute() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                $extra['SELECT'] .= ',(SELECT CONCAT(st.LAST_NAME,\', \',coalesce(st.FIRST_NAME)) FROM staff st,course_periods cp,course_period_var cpv,institute_periods p,schedule ss WHERE st.STAFF_ID=cp.TEACHER_ID AND cpv.PERIOD_id=p.PERIOD_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID  AND p.ATTENDANCE=\'Y\' AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.STUDENT_ID=s.STUDENT_ID AND ss.SYEAR=\'' . UserSyear() . '\' AND ss.MARKING_PERIOD_ID = \'' . UserMp() . '\' AND (ss.START_DATE<=\'' . DBDate() . '\' AND (ss.END_DATE>=\'' . DBDate() . '\' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS TEACHER';
                $extra['SELECT'] .= ',(SELECT cpv.ROOM_ID AS ROOM FROM course_periods cp,course_period_var cpv,institute_periods p,schedule ss WHERE cpv.PERIOD_id=p.PERIOD_ID AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND p.ATTENDANCE=\'Y\' AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.STUDENT_ID=s.STUDENT_ID AND ss.SYEAR=\'' . UserSyear() . '\' AND ss.MARKING_PERIOD_ID = \'' . UserMp() . '\' AND (ss.START_DATE<=\'' . DBDate() . '\' AND (ss.END_DATE>=\'' . DBDate() . '\' OR ss.END_DATE IS NULL)) ORDER BY p.SORT_ORDER LIMIT 1) AS ROOM';
            }
            $extra['ORDER_BY'] = 'HONOR_ROLL,SORT_ORDER DESC,ROOM,FULL_NAME';
        ...
        if ($_REQUEST['list']) {
            ...
            echo '<CENTER>';
            echo '<TABLE width=80%>';
            echo '<TR align=center><TD colspan=6><B>' . sprintf(('%s Honor Roll'), $institute_info_RET[1]['TITLE']) . ' </B></TD></TR>';
            ...
            foreach ($RET as $student) {
                ...
                echo '<TR align=center><TD><FONT size=3>' . ''._whoHasCompletedAllTheAcademic.'<BR>'._requirementsFor.'<BR>' . $institute_info_RET[1]['TITLE'] . ' ' . ($student['HONOR_ROLL']) . ' '._honorRoll.'</FONT></TD><TR>';
                echo '</TABLE>';
                ...
                echo '<TR><TD><FONT size=4>' . $institute_info_RET[1]['PRINCIPAL'] . '<BR></FONT><FONT size=0>'._principal.'</FONT></TD>';
...
if (!$_REQUEST['modfunc']) {
    if ($_REQUEST['search_modfunc'] == 'list') {
        ...
        if ($_REQUEST['w_course_period_id'])
            echo "<FORM action=ForExport.php?modname=" . strip_tags(trim($_REQUEST[modname])) . "&modfunc=save&include_inactive=" . strip_tags(trim($_REQUEST['include_inactive'])) . "&honor_roll=" . strip_tags(trim($_REQUEST['honor_roll'])) . "&mp=$mp&w_course_period_id=" . strip_tags(trim($_REQUEST['w_course_period_id'])) . "&HaniIMS_PDF=true method=POST target=_blank>";
        else
            echo "<FORM action=ForExport.php?modname=" . strip_tags(trim($_REQUEST[modname])) . "&modfunc=save&include_inactive=" . strip_tags(trim($_REQUEST['include_inactive'])) . "&honor_roll=" . strip_tags(trim($_REQUEST['honor_roll'])) . "&mp=$mp&HaniIMS_PDF=true method=POST target=_blank>";
...
function MyWidgets($item, $mp) {
    ...
    switch ($item) {
        case 'honor_roll':
            ...
            if ($_REQUEST['honor_roll'] != 986 && $_REQUEST['honor_roll']) {
                ...
                if ($_REQUEST['w_course_period_id'])
                    $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                ...
                                                                and srcg.`STUDENT_ID`=ssm.STUDENT_ID) order by hr.value asc limit 1),(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE<=(SELECT ROUND(AVG(srcg.grade_percent)) FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.course_period_id=\'' . $_REQUEST['w_course_period_id'] . '\' and cpp.does_honor_roll=\'Y\'
                                                                and srcg.`STUDENT_ID`=ssm.STUDENT_ID) order by hr.value desc limit 1))
                                                                FROM `student_report_card_grades` srcg,course_periods cpp WHERE srcg.MARKING_PERIOD_ID = ' . UserMp() . ' and srcg.course_period_id=cpp.course_period_id  and cpp.does_honor_roll="Y" and srcg.`STUDENT_ID`=ssm.STUDENT_ID)  AND hr.INSTITUTE_ID=' . UserInstitute() . ' AND hr.SYEAR=' . UserSyear() . ')AS HONOR_ROLL';
                else
                    $extra['SELECT'] .= ',(SELECT hr.TITLE FROM honor_roll hr WHERE hr.VALUE=
                                                                (SELECT if(
                                                                            (ROUND(AVG(srcg.grade_percent))>=(SELECT hr.VALUE FROM honor_roll hr WHERE hr.VALUE>=
            ...
            $option = DBGet(DBQuery('SELECT TITLE,VALUE  FROM honor_roll WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\'  ORDER BY VALUE'));
            $options['986'] = 'All';
