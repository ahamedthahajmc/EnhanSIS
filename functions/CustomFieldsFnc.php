				...
				foreach ($_REQUEST['cust'] as $id => $value) {
					...
					if ($value != '') {
						switch ($fields[$id][1]['TYPE']) {
							case 'radio':
								$_HaniIMS['SearchTerms'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ': </b></font>';
								if ($value == 'Y') {
									$string .= ' and s.' . $field_name . '=\'' . $value . '\' ';
									$_HaniIMS['SearchTerms'] .= 'Yes';
								} elseif ($value == 'N') {
									$string .= ' and (s.' . $field_name . '!=\'Y\' OR s.' . $field_name . ' IS NULL) ';
									$_HaniIMS['SearchTerms'] .= 'No';
								}
							...
							case 'codeds':
								...
								if ($value == '!') {
									...
									$_HaniIMS['SearchTerms'] .= 'No Value';
								} else {
									$string .= ' and s.' . $field_name . '=\'' . $value . '\' ';
									$_HaniIMS['SearchTerms'] .= $value;
								}
								$_HaniIMS['SearchTerms'] .= '<BR>';
								break;
							...
							case 'text':
								if (substr($value, 0, 2) == '\"' && substr($value, -2) == '\"') {
									...
									$_HaniIMS['SearchTerms'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ': </b></font>' . substr($value, 2, -2) . '<BR>';
								} else {
									$string .= ' and LOWER(s.' . $field_name . ') LIKE \'' . strtolower($value) . '%\' ';
									if ($exp == 1)
										$_HaniIMS['Search'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ' starts with: </b></font>' . $value . '<BR>';
									elseif ($exp == 2) {
										$_HaniIMS['SearchTerms'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ' starts with: </b></font>' . $value . '<BR>';
									} else {
										$_HaniIMS['SearchTerms'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ' starts with: </b></font>' . $value . '<BR>';
									}
								}
				...
				foreach ($_REQUEST['cust_begin'] as $id => $value) {
					...
					if ($_REQUEST['cust_begin'][$field_name] != '' && $_REQUEST['cust_end'][$field_name] != '') {
						...
						if ($fields[$id][1]['TYPE'] == 'date')
							$_HaniIMS['SearchTerms'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ' between: </b></font>' . date('M/d/Y', strtotime($value)) . ' &amp; ' . date('M/d/Y', strtotime($_REQUEST['cust_end'][$field_name])) . '<BR>';
						else
							$_HaniIMS['SearchTerms'] .= '<font color=gray><b>' . $fields[$id][1]['TITLE'] . ' between: </b></font>' . $value . ' &amp; ' . $_REQUEST['cust_end'][$field_name] . '<BR>';
					}
				}
			}
