...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'choose_course') {
    ...
    if (count($RET)) {
        ...
        $teachers_RET = DBGet(DBQuery("SELECT concat((COALESCE(LAST_NAME,' '), ', ', COALESCE(FIRST_NAME,' '), ' ', COALESCE(MIDDLE_NAME,' '))) as Teacher FROM staff WHERE (INSTITUTES IS NULL OR strpos(INSTITUTES,'," . UserInstitute() . ",')>0) AND SYEAR='" . UserSyear() . "' AND PROFILE='teacher' and STAFF_ID='" . $RET['TEACHER_ID'] . "' ORDER BY LAST_NAME,FIRST_NAME"));

        $header .= '<TD><b>' . $teachers_RET[1]['TEACHER'] . '</b><br>Teacher</TD>';
        $header .= '<TD><b>' . $RET['ROOM'] . '</b><br>' . _location . '</TD>';
        $sql = "SELECT TITLE,START_TIME,END_TIME FROM institute_periods WHERE PERIOD_ID='" . $RET['PERIOD_ID'] . "'";
        ...
        $mp_RET = DBGet(DBQuery("SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,'2' AS TABLE,SORT_ORDER FROM institute_quarters WHERE INSTITUTE_ID='" . UserInstitute() . "' AND SYEAR='" . UserSyear() . "' and marking_period_id='" . $mp . "' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,'1' AS TABLE,SORT_ORDER FROM institute_semesters WHERE INSTITUTE_ID='" . UserInstitute() . "' AND SYEAR='" . UserSyear() . "' and marking_period_id='" . $mp . "' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,'0' AS TABLE,SORT_ORDER FROM institute_years WHERE INSTITUTE_ID='" . UserInstitute() . "' AND SYEAR='" . UserSyear() . "' and marking_period_id='" . $mp . "' ORDER BY 3,4"));
        ...
        if ($RET['GRADE_SCALE_ID'] != '') {
            $sql = "SELECT TITLE,ID FROM report_card_grade_scales WHERE SYEAR='" . UserSyear() . "' AND INSTITUTE_ID='" . UserInstitute() . "' and ID='" . $RET['GRADE_SCALE_ID'] . "'";
        ...
        if ($RET['CALENDAR_ID'] != '') {
            $sql = "SELECT TITLE,CALENDAR_ID FROM institute_calendars WHERE CALENDAR_ID='" . $RET['CALENDAR_ID'] . "'";
    ...
    if ($_REQUEST['print'] != 'list') {
        echo "<FORM name=search  class=form-horizontal id=search action=ForExport.php?modname=$_REQUEST[modname]&modfunc=print&marking_period_id=" . $_REQUEST['marking_period_id'] . "&HaniIMS_PDF=true&report=true&print=list method=POST target=_blank>";
    ...
    $mp_RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'2\'  FROM institute_quarters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'1\' FROM institute_semesters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'0\' FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' ORDER BY 3,4'));
    unset($options);
    if (count($mp_RET)) {
        ...
        if ($_REQUEST['marking_period_id'] && $_REQUEST['marking_period_id'] != '') {
            ...
            if (count($subjects_RET)) {
                ...
                if ($_REQUEST['subject_id'] && $_REQUEST['subject_id'] != '') {
                    ...
                    $sql = 'SELECT COURSE_ID,TITLE FROM courses WHERE SUBJECT_ID=\'' . $_REQUEST['subject_id'] . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY TITLE';
                    $QI = DBQuery($sql);
...
function CreateList($dli = '', $pli = '', $sli = '', $cli = '', $mp = '', $mp_name = '')
{
    ...
    if ($mp != '') {
        $sql = 'SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'2\'  FROM institute_quarters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' and marking_period_id=\'' . $mp . '\' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'1\' FROM institute_semesters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' and marking_period_id=\'' . $mp . '\' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'0\'  FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' and marking_period_id=\'' . $mp . '\' ORDER BY 3,4';
    ...
    if ($mp == '') {
        ...
        if ($sli == '') {
            ...
            if ($cli == '') {
                $where = 'and marking_period_id=\'' . $mp . '\' and course_id in (select Course_Id from courses where subject_id = \'' . $_REQUEST['subject_id'] . '\' and Institute_Id=\'' . UserInstitute() . '\')';
    ...
    short_name,(select CONCAT(START_TIME,\' - \',END_TIME,\' \') from institute_periods where period_id=course_period_var.period_id) as period_time, (select title from institute_periods where period_id=course_period_var.period_id) as period, marking_period_id, course_periods.course_period_id as mp,
    (select CONCAT(LAST_NAME,\' \',FIRST_NAME,\' \') from staff where staff_id=course_periods.teacher_id) as teacher, rooms.title as location,days,course_periods.course_period_id,course_periods.begin_date,course_periods.end_date,course_periods.SCHEDULE_TYPE,course_period_var.ID AS CPV_ID from course_periods,course_period_var,rooms where course_periods.institute_id=\'' . UserInstitute() . '\' and course_period_var.room_id=rooms.room_id and course_periods.course_period_id=course_period_var.course_period_id and course_periods.syear=\'' . UserSyear() . '\' ' . $where . '  GROUP BY course_period_var.COURSE_PERIOD_ID ORDER BY course_period_var.ID';
    ...
    foreach ($ret_temp as $ri => $rd) {
        ...
        if ($rd['SCHEDULE_TYPE'] == 'FIXED') {
            ...
            $get_det = DBGet(DBQuery('SELECT cpv.*,CONCAT(sp.START_TIME,\' - \',sp.END_TIME,\' \') as PERIOD_TIME,sp.TITLE as PERIOD,r.TITLE AS LOCATION FROM course_period_var cpv,institute_periods sp,rooms r WHERE cpv.COURSE_PERIOD_ID=' . $rd['COURSE_PERIOD_ID'] . ' AND cpv.PERIOD_ID=sp.PERIOD_ID AND cpv.ROOM_ID=r.ROOM_ID'));
            $ret[$i] = $rd;
            if (count($get_det) > 0) {
    ...
    $html = "<b>" . $heading . "</b><br>
        <A HREF=" . str_replace('Modules.php', 'ForExport.php', $PHP_tmp_SELF) . "&create_excel=true&LO_save=1&HaniIMS_PDF=true > <IMG SRC=assets/download.png border=0 vspace=0 hspace=0></A>
        <br>";
    $html .= "<table width=100%  style=\" font-size:12px;\" >";
    $html .= "<tr><td  style=\"font-size:15px; font-weight:bold;\">" . GetInstitute(UserInstitute()) . "<div style=\"font-size:12px;\">" . _studentSchedulesReport . "</div></td><td align=right style=\"padding-top:10px;\">" . ProperDate(DBDate()) . "<br />" . _poweredBy . " hani</td></tr><tr><td colspan=2 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
    ...
    if ($_REQUEST['print'] == 'list') {
        ...
        echo "<tr><td  style=\"font-size:15px; font-weight:bold;\">" . GetInstitute(UserInstitute()) . "<div style=\"font-size:12px;\">" . _courseCatalog . "</div></td><td align=right style=\"padding-top:5px;\">" . ProperDate(DBDate()) . "<br />" . _poweredBy . " hani</td></tr><tr><td colspan=2 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
    }
...
function CreateExcel($dli = '', $pli = '', $sli = '', $cli = '', $mp = '', $mp_name = '')
{
    ...
    if ($mp != '') {
        $sql = 'SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'2\' AS TABLE,SORT_ORDER FROM institute_quarters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' and marking_period_id=\'' . $mp . '\' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'1\' AS TABLE,SORT_ORDER FROM institute_semesters WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' and marking_period_id=\'' . $mp . '\' UNION SELECT MARKING_PERIOD_ID,TITLE,SHORT_NAME,\'0\' AS TABLE,SORT_ORDER FROM institute_years WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' and marking_period_id=\'' . $mp . '\' ORDER BY 3,4';
    ...
    $sql = 'select
                ...
                (select title from course_subjects where subject_id=(select subject_id from courses where 						course_id=course_periods.course_id)) as subject,
                short_name,
                (select title from institute_periods where period_id=cpv.period_id) as period,
                marking_period_id,
                ...
                from course_periods,cours_period_var cpv,rooms r where course_period.institute_id=\'' . UserInstitute() . '\' and course_period.course_period_id=cpv.course_period_id and r.room_id=cpv.room_id and course_period.syear=\'' . UserSyear() . '\' ' . $where . '';
