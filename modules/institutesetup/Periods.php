    ...
    foreach ($_REQUEST['values'] as $id => $columns) {
        if (!(isset($columns['TITLE']) && trim($columns['TITLE']) == '')) {
            ...
            if ($id != 'new' && $columns['START_TIME'] != '' && !isset($columns['END_TIME'])) {
                $period_etime = DBGet(DBQuery('SELECT END_TIME FROM  institute_periods WHERE period_id=\'' . $id . '\''));
            ...
            if ($id != 'new' && $columns['END_TIME'] != '' && !isset($columns['START_TIME'])) {
                $period_etime = DBGet(DBQuery('SELECT START_TIME FROM  institute_periods WHERE period_id=\'' . $id . '\''));
            ...
            if ($id != 'new') {
                $exist_pr = DBGet(DBQuery('SELECT * FROM  institute_periods WHERE period_id=\'' . $id . '\''));
                if (isset($_REQUEST['values'][$id]['TITLE']) && $_REQUEST['values'][$id]['TITLE'] != '' || isset($_REQUEST['values'][$id]['SHORT_NAME']) && $_REQUEST['values'][$id]['SHORT_NAME'] != '') {
                    ...
                    $sql = 'SELECT TITLE,SHORT_NAME,SORT_ORDER,START_TIME,END_TIME FROM  institute_periods WHERE SYEAR= \'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' and period_id<>\'' . $id . '\'';
                ...
                if ((isset($_REQUEST['values'][$id]['TITLE']) && $_REQUEST['values'][$id]['TITLE'] != $exist_pr[1]['TITLE']) || (isset($_REQUEST['values'][$id]['SHORT_NAME']) && $_REQUEST['values'][$id]['SHORT_NAME'] != $exist_pr[1]['SHORT_NAME']) || (isset($_REQUEST['values'][$id]['SORT_ORDER']) && $_REQUEST['values'][$id]['SORT_ORDER'] != $exist_pr[1]['SORT_ORDER']) || (isset($_REQUEST['values'][$id]['START_TIME']) && strtotime($_REQUEST['values'][$id]['START_TIME']) != strtotime($exist_pr[1]['START_TIME'])) || (isset($_REQUEST['values'][$id]['END_TIME']) && strtotime($_REQUEST['values'][$id]['END_TIME']) != strtotime($exist_pr[1]['END_TIME'])) || (isset($_REQUEST['values'][$id]['IGNORE_SCHEDULING']) && $_REQUEST['values'][$id]['IGNORE_SCHEDULING'] != $exist_pr[1]['IGNORE_SCHEDULING']) || (isset($_REQUEST['values'][$id]['ATTENDANCE']) && $_REQUEST['values'][$id]['ATTENDANCE'] != $exist_pr[1]['ATTENDANCE'])) {
                    $sql = 'UPDATE institute_periods SET ';
                    $title_change = '';
                    foreach ($columns as $column => $value) {
                        ...
                        if ($column == 'ignore_scheduling' && $value == '') {
                            ...
                            if ($checker[1]['TOTAL'] == 0) {
                                ...
                                $check_for_change = DBGet(DBQuery('SELECT COUNT(*) AS REC_EX FROM institute_periods WHERE PERIOD_ID=' . $id . ' AND ' . $column . '=\'' . $value . '\''));
                            ...
                            if ($column == 'TITLE' && !isset($_REQUEST['values'][$id]['SHORT_NAME'])) {
                                $exist_pr_title = DBGet(DBQuery('SELECT count(PERIOD_ID) AS TOT FROM  institute_periods WHERE title=\'' . $value . '\' AND SHORT_NAME=\'' . $exist_pr[1]['SHORT_NAME'] . '\' AND SYEAR= \'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND period_id<>\'' . $id . '\''));
                            ...
                            if ($column == 'SHORT_NAME' && !isset($_REQUEST['values'][$id]['TITLE'])) {
                                $exist_pr_title = DBGet(DBQuery('SELECT count(PERIOD_ID) AS TOT FROM  institute_periods WHERE title=\'' . $exist_pr[1]['TITLE'] . '\' AND SHORT_NAME=\'' . $value . '\' AND SYEAR= \'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND period_id<>\'' . $id . '\''));
                    ...
                    $sql_get_length = 'SELECT start_time, end_time from institute_periods WHERE period_id=\'' . $id . '\'';
                    ...
                    $sql_length_update = 'UPDATE institute_periods set length = ' . $length . ' where period_id=\'' . $id . '\'';
                ...
                $sql = 'SELECT TITLE,SHORT_NAME,SORT_ORDER,START_TIME,END_TIME FROM  institute_periods WHERE SYEAR= \'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\'';
                $periods = DBGET(DBQuery($sql));
                //$end_time = array();
                for ($i = 1; $i <= count($periods); $i++) {
                    $shortname[$i] = strtoupper(str_replace(' ', '', $periods[$i]['SHORT_NAME']));
                ...
                if (is_array($p_title) && in_array(strtoupper(str_replace(' ', '', $columns['TITLE'])), $p_title) || is_array($shortname) && in_array(strtoupper(str_replace(' ', '', $columns['SHORT_NAME'])), $shortname)) {
                    ...
                    $sql = 'INSERT INTO institute_periods ';
                    ...
                    $p_id = DBGet(DBQuery('SELECT max(PERIOD_ID) AS period_id FROM institute_periods WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\''));
                    $period_id = $p_id[1]['PERIOD_ID'];

                    $time_chk = DBGet(DBQuery('SELECT START_TIME,END_TIME FROM institute_periods WHERE PERIOD_ID=\'' . $period_id . '\' AND SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\''));
                    ...
                    $sql_up = 'update institute_periods set length = ' . $length . ' where period_id=\'' . $period_id . '\' and syear=\'' . UserSyear() . '\' and institute_id=\'' . UserInstitute() . '\'';
...
DrawBC("" . _instituteSetup . " <i class=\"icon-arrow-right13\"></i> " . ProgramTitle());
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'remove' && AllowEdit()) {
    ...
    if ($has_assigned > 0) {
        ...
        if (DeletePrompt_Period('period')) {
            DBQuery('DELETE FROM institute_periods WHERE PERIOD_ID=\'' . $prd_id . '\'');
...
if ($_REQUEST['modfunc'] != 'remove') {
    ...
    $sql = 'SELECT PERIOD_ID,TITLE,SHORT_NAME,SORT_ORDER,LENGTH,START_TIME,END_TIME,ATTENDANCE AS UA,IGNORE_SCHEDULING AS IGS,ATTENDANCE,IGNORE_SCHEDULING FROM institute_periods WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY SORT_ORDER';
    $QI = DBQuery($sql);
    ...
    if ($count != 0) {
        $maxPeriodId = DBGet(DBQuery("select max(PERIOD_ID) as maxPeriodId from institute_periods WHERE SYEAR='" . UserSyear() . "' AND INSTITUTE_ID='" . UserInstitute() . "'"));
    ...
    echo '<hr class="no-margin"/><div class="panel-body text-right">' . SubmitButton(_save, '', 'id="setupPeriodsBtn" class="btn btn-primary" onclick="formcheck_institute_setup_periods(this);"') . '</div>';
    echo '</div>';
    echo '</FORM>';
}
...
function _makeTimeInput($value, $name)
{
    ...
    if ($id != 'new') {
        $sql_ampm_s = 'SELECT START_TIME FROM institute_periods WHERE period_id=' . $id;
...
function _makeTimeInputEnd($value, $name)
{
    ...
    if ($id != 'new') {
        $sql_ampm = 'select end_time from institute_periods where period_id=' . $id;
