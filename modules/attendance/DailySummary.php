...
if ($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax']) && AllowEdit()) {
    foreach ($_REQUEST['attendance'] as $student_id => $values) {
        foreach ($values as $institute_date => $columns) {
            $sql = 'UPDATE attendance_period SET ADMIN=\'Y\',';
            foreach ($columns as $column => $value)
                $sql .= $column . "='" . str_replace("\'", "''", $value) . "',";

            $sql = substr($sql, 0, -1) . ' WHERE INSTITUTE_DATE=\'' . date('Y-m-d', strtotime($institute_date)) . '\' AND PERIOD_ID=\'' . optional_param('period_id', '', PARAM_SPCL) . '\' AND STUDENT_ID=\'' . $student_id . '\'';
            DBQuery($sql);
            UpdateAttendanceDaily($student_id, $institute_date);
        }
    }

    $current_RET = DBGet(DBQuery('SELECT ATTENDANCE_TEACHER_CODE,ATTENDANCE_CODE,ATTENDANCE_REASON,STUDENT_ID,ADMIN,COURSE_PERIOD_ID FROM attendance_period WHERE INSTITUTE_DATE=\'' . date('Y-m-d', strtotime($date)) . '\''), array(), array('STUDENT_ID', 'COURSE_PERIOD_ID'));
...
if ($_REQUEST['search_modfunc'] || $_REQUEST['student_id'] || UserStudentID() || User('PROFILE') == 'parent' || User('PROFILE') == 'student') {
    ...
    if (!UserStudentID() && !$_REQUEST['student_id']) {
        $periods_RET = DBGet(DBQuery('SELECT PERIOD_ID,TITLE FROM institute_periods WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY SORT_ORDER'));
...
$cal_RET = DBGet(DBQuery('SELECT DISTINCT INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_calendar WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\' ORDER BY INSTITUTE_DATE'));
...
if (UserStudentID() || $_REQUEST['student_id'] || User('PROFILE') == 'parent') {
    ...
    if ($_REQUEST['period_id']) {
        $sql = 'SELECT cp.TITLE as COURSE_PERIOD,sp.TITLE as PERIOD,cpv.PERIOD_ID, cp.COURSE_PERIOD_ID
                 FROM schedule s,courses c,course_periods cp,course_period_var cpv,institute_periods sp
                 WHERE
        ...
        if ($_REQUEST['myclasses'] != '') {
            $sql = 'SELECT ap.INSTITUTE_DATE,ap.COURSE_PERIOD_ID,ac.SHORT_NAME,ac.STATE_CODE,ac.DEFAULT_CODE FROM attendance_period ap,attendance_codes ac, course_periods cp WHERE ap.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND ' . (($_REQUEST['myclasses'] == 'my_classes') ? "(cp.TEACHER_ID='" . User('STAFF_ID') . "' OR cp.SECONDARY_TEACHER_ID='" . User('STAFF_ID') . "')" : "cp.COURSE_PERIOD_ID='" . UserCoursePeriod() . "'") . " AND ap.INSTITUTE_DATE BETWEEN '" . date('Y-m-d', strtotime($start_date)) . "' AND '" . date('Y-m-d', strtotime($end_date)) . "' AND ap.ATTENDANCE_CODE=ac.ID AND ap.STUDENT_ID='" . UserStudentID() . "'";
        } else {
            $sql = 'SELECT ap.INSTITUTE_DATE,ap.COURSE_PERIOD_ID,ac.SHORT_NAME,ac.STATE_CODE,ac.DEFAULT_CODE FROM attendance_period ap,attendance_codes ac WHERE ap.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\' AND ap.ATTENDANCE_CODE=ac.ID AND ap.STUDENT_ID=\'' . UserStudentID() . '\'';
        }
        $attendance_RET = DBGet(DBQuery($sql), array(), array('INSTITUTE_DATE', 'COURSE_PERIOD_ID'));
    } else {
        $_REQUEST['myclasses'] = '';
        $schedule_RET[1] = array('COURSE_PERIOD' => _dailyAttendance, 'COURSE_PERIOD_ID' => '0');
        if ($_REQUEST['myclasses'] != '') {
            $attendance_RET = DBGet(DBQuery('SELECT ad.INSTITUTE_DATE,\'0\' AS COURSE_PERIOD_ID,ad.STATE_VALUE AS STATE_CODE,' . db_case(array('ad.STATE_VALUE', "'0.0'", "'A'", "'1.0'", "'P'", "'H'")) . ' AS SHORT_NAME FROM attendance_day ad, attendance_period ap, course_periods cp WHERE ad.STUDENT_ID=ap.STUDENT_ID AND ad.INSTITUTE_DATE=ap.INSTITUTE_DATE AND ap.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND ' . (($_REQUEST['myclasses'] == 'my_classes') ? '(cp.TEACHER_ID=\'' . User('STAFF_ID') . '\' OR cp.SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\')' : 'cp.COURSE_PERIOD_ID=\'' . UserCoursePeriod() . '\'') . ' AND ad.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\' AND ad.STUDENT_ID=\'' . UserStudentID() . '\''), array(), array('INSTITUTE_DATE', 'COURSE_PERIOD_ID'));
        } else {
            $attendance_RET = DBGet(DBQuery('SELECT ad.INSTITUTE_DATE,\'0\' AS COURSE_PERIOD_ID,ad.STATE_VALUE AS STATE_CODE,' . db_case(array('ad.STATE_VALUE', "'0.0'", "'A'", "'1.0'", "'P'", "'H'")) . ' AS SHORT_NAME FROM attendance_day ad WHERE ad.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\' AND ad.STUDENT_ID=\'' . UserStudentID() . '\''), array(), array('INSTITUTE_DATE', 'COURSE_PERIOD_ID'));
        }
    }
    ...
    if (count($schedule_RET)) {
        foreach ($schedule_RET as $course) {
            ...
            foreach ($cal_RET as $value)
                $student_RET[$i][$value['SHORT_DATE']] = _makePeriodColor($attendance_RET[$value['INSTITUTE_DATE']][$course['COURSE_PERIOD_ID']][1]['SHORT_NAME'], $attendance_RET[$value['INSTITUTE_DATE']][$course['COURSE_PERIOD_ID']][1]['STATE_CODE'], $attendance_RET[$value['INSTITUTE_DATE']][$course['COURSE_PERIOD_ID']][1]['DEFAULT_CODE']);
        }
    }
    ...
    if (count($cal_RET)) {
        foreach ($cal_RET as $value)
            $columns[$value['SHORT_DATE']] = ShortDate($value['INSTITUTE_DATE']);
    }
...
//working
        if ($_REQUEST['myclasses'] != '') {
            if ($_REQUEST['include_inactive'] == 'Y') {
                $sql = 'SELECT ad.STATE_VALUE,ad.STUDENT_ID,ad.INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ad.INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_day ad,student_enrollment ssm, attendance_period ap, course_periods cp WHERE ad.STUDENT_ID=ap.STUDENT_ID AND ad.INSTITUTE_DATE=ap.INSTITUTE_DATE AND ap.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND ' . (($_REQUEST['myclasses'] == 'my_classes') ? '(cp.TEACHER_ID=\'' . User('STAFF_ID') . '\' OR cp.SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\')' : 'cp.COURSE_PERIOD_ID=\'' . UserCoursePeriod() . '\'') . ' AND ad.STUDENT_ID=ssm.STUDENT_ID AND \'' . date('Y-m-d', strtotime(DBDate())) . '\'>=ssm.START_DATE AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND ad.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\'';
            } else {
                $sql = 'SELECT ad.STATE_VALUE,ad.STUDENT_ID,ad.INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ad.INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_day ad,student_enrollment ssm, attendance_period ap, course_periods cp WHERE ad.STUDENT_ID=ap.STUDENT_ID AND ad.INSTITUTE_DATE=ap.INSTITUTE_DATE AND ap.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND ' . (($_REQUEST['myclasses'] == 'my_classes') ? '(cp.TEACHER_ID=\'' . User('STAFF_ID') . '\' OR cp.SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\')' : 'cp.COURSE_PERIOD_ID=\'' . UserCoursePeriod() . '\'') . ' AND ad.STUDENT_ID=ssm.STUDENT_ID AND (\'' . date('Y-m-d', strtotime(DBDate())) . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR ssm.END_DATE IS NULL) AND \'' . date('Y-m-d', strtotime(DBDate())) . '\'>=ssm.START_DATE AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND ad.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\'';
            }
        } else {
            if ($_REQUEST['include_inactive'] == 'Y') {
                $sql = 'SELECT ad.STATE_VALUE,ad.STUDENT_ID,INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ad.INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_day ad,student_enrollment ssm WHERE ad.STUDENT_ID=ssm.STUDENT_ID AND \'' . date('Y-m-d', strtotime(DBDate())) . '\'>=ssm.START_DATE AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\'';
            } else {
                $sql = 'SELECT ad.STATE_VALUE,ad.STUDENT_ID,INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ad.INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_day ad,student_enrollment ssm WHERE ad.STUDENT_ID=ssm.STUDENT_ID AND (\'' . date('Y-m-d', strtotime(DBDate())) . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR ssm.END_DATE IS NULL) AND \'' . date('Y-m-d', strtotime(DBDate())) . '\'>=ssm.START_DATE AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\'';
            }
        }
        ...
        if ($_REQUEST['myclasses'] != '') {
            $sql = 'SELECT ap.ATTENDANCE_CODE,ap.STUDENT_ID,ap.INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ap.INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_period ap,student_enrollment ssm,students s,course_periods cp WHERE ap.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND s.STUDENT_ID=ssm.STUDENT_ID AND ap.STUDENT_ID=ssm.STUDENT_ID AND ' . (($_REQUEST['myclasses'] == 'my_classes') ? '(cp.TEACHER_ID=\'' . User('STAFF_ID') . '\' OR cp.SECONDARY_TEACHER_ID=\'' . User('STAFF_ID') . '\')' : 'cp.COURSE_PERIOD_ID=\'' . UserCoursePeriod() . '\'') . ' AND ap.PERIOD_ID=\'' . $_REQUEST['period_id'] . '\' AND ap.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\'';
        } else {
            $sql = 'SELECT ap.ATTENDANCE_CODE,ap.STUDENT_ID,ap.INSTITUTE_DATE,CONCAT(\'_\',DATE_FORMAT(ap.INSTITUTE_DATE,\'%Y%m%d\')) AS SHORT_DATE FROM attendance_period ap,student_enrollment ssm,students s WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ap.STUDENT_ID=ssm.STUDENT_ID AND ap.INSTITUTE_DATE BETWEEN \'' . date('Y-m-d', strtotime($start_date)) . '\' AND \'' . date('Y-m-d', strtotime($end_date)) . '\' AND ap.PERIOD_ID=\'' . $_REQUEST['period_id'] . '\'';
        }
        if ($_REQUEST['include_inactive'] != 'Y') {
...
// TODO Do not Delete       $sql = appendSQL($sql,$tmp_extra=array('NoSearchTerms'=>true)); // extra must be lvalue
    ...
    if (count($cal_RET)) {
        foreach ($cal_RET as $value) {
            $extra['SELECT'] .= ",'' as _" . str_replace('-', '', $value['INSTITUTE_DATE']);
            $extra['columns_after']['_' . str_replace('-', '', $value['INSTITUTE_DATE'])] = ShortDate($value['INSTITUTE_DATE']);
            $extra['functions']['_' . str_replace('-', '', $value['INSTITUTE_DATE'])] = '_makeColor';
...
function _makeColor($value, $column) {
    ...
    if ($_REQUEST['period_id']) {
        if (!$attendance_codes)
            $attendance_codes = DBGet(DBQuery('SELECT ID,TITLE,DEFAULT_CODE,STATE_CODE,SHORT_NAME FROM attendance_codes WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND TABLE_NAME=\'0\''), array(), array('ID'));
...
function makeCodePulldown($value, $student_id, $date) {
    global $THIS_RET, $attendance_codes, $_HaniIMS;

    $date = substr($date, 1, 4) . '-' . substr($date, 5, 2) . '-' . substr($date, 7);

    if (!$_HaniIMS['code_options']) {
        foreach ($attendance_codes as $id => $code)
            $_HaniIMS['code_options'][$id] = ($code[1]['SHORT_NAME'] == '' ? $code[1]['TITLE'] : $code[1]['SHORT_NAME']);
    }

    return SelectInput($value, 'attendance[' . $student_id . '][' . $date . '][ATTENDANCE_CODE]', '', $_HaniIMS['code_options']);
}
