...
if(User('PROFILE')=='admin'&& isset($_REQUEST['action']) && $_REQUEST['action']=='Backup' )
{
    ...
    $backup_folder = 'HaniIMS_databackup';
    if(!is_dir($backup_folder))
    {
      mkdir("HaniIMS_databackup");
    }

    $Export_FileName = 'HaniIMS_databackup/'.$mysql_database . 'Backup' . $date_time . '.sql';
...
function _mysqldump($mysql_database)
{
	...
	$f_content_n.= "--
            ...
            SELECT q.marking_period_id, 'HaniIMS' AS mp_source, q.syear,
        ...
        UNION
            SELECT marking_period_id, 'HaniIMS' AS mp_source, syear,
        ...
        UNION
            ...
            SELECT cp.institute_id, cp.syear, cp.marking_period_id, cp.period_id, c.subject_id,
...
CREATE PROCEDURE `ATTENDANCE_CALC`(IN cp_id INT,IN year INT,IN institute INT)
BEGIN
DELETE FROM missing_attendance WHERE COURSE_PERIOD_ID=cp_id;
INSERT INTO missing_attendance(INSTITUTE_ID,SYEAR,INSTITUTE_DATE,COURSE_PERIOD_ID,PERIOD_ID,TEACHER_ID,SECONDARY_TEACHER_ID) SELECT s.ID AS INSTITUTE_ID,acc.SYEAR,acc.INSTITUTE_DATE,cp.COURSE_PERIOD_ID,cp.PERIOD_ID, IF(tra.course_period_id=cp.course_period_id AND acc.institute_date<tra.assign_date =true,tra.pre_teacher_id,cp.teacher_id) AS TEACHER_ID,cp.SECONDARY_TEACHER_ID FROM attendance_calendar acc INNER JOIN marking_periods mp ON mp.SYEAR=acc.SYEAR AND mp.INSTITUTE_ID=acc.INSTITUTE_ID AND acc.INSTITUTE_DATE BETWEEN mp.START_DATE AND mp.END_DATE INNER JOIN course_periods cp ON cp.MARKING_PERIOD_ID=mp.MARKING_PERIOD_ID AND cp.DOES_ATTENDANCE='Y' AND cp.CALENDAR_ID=acc.CALENDAR_ID LEFT JOIN teacher_reassignment tra ON (cp.course_period_id=tra.course_period_id) INNER JOIN institute_periods sp ON sp.SYEAR=acc.SYEAR AND sp.INSTITUTE_ID=acc.INSTITUTE_ID AND sp.PERIOD_ID=cp.PERIOD_ID AND (sp.BLOCK IS NULL AND position(substring('UMTWHFS' FROM DAYOFWEEK(acc.INSTITUTE_DATE) FOR 1) IN cp.DAYS)>0 OR sp.BLOCK IS NOT NULL AND acc.BLOCK IS NOT NULL AND sp.BLOCK=acc.BLOCK) INNER JOIN institutes s ON s.ID=acc.INSTITUTE_ID INNER JOIN schedule sch ON sch.COURSE_PERIOD_ID=cp.COURSE_PERIOD_ID AND sch.START_DATE<=acc.INSTITUTE_DATE AND (sch.END_DATE IS NULL OR sch.END_DATE>=acc.INSTITUTE_DATE ) AND cp.COURSE_PERIOD_ID= cp_id LEFT JOIN attendance_completed ac ON ac.INSTITUTE_DATE=acc.INSTITUTE_DATE AND IF(tra.course_period_id=cp.course_period_id AND acc.institute_date<=tra.assign_date =true,ac.staff_id=tra.pre_teacher_id,ac.staff_id=cp.teacher_id) AND ac.PERIOD_ID=sp.PERIOD_ID WHERE acc.SYEAR=year AND acc.INSTITUTE_ID=institute AND (acc.MINUTES IS NOT NULL AND acc.MINUTES>0) AND acc.INSTITUTE_DATE<=CURDATE() AND ac.STAFF_ID IS NULL  AND isDateInMarkingPeriodWorkingDates(cp.marking_period_id, acc.INSTITUTE_DATE) GROUP BY s.TITLE,acc.INSTITUTE_DATE,cp.TITLE,cp.COURSE_PERIOD_ID,cp.TEACHER_ID;
END$$
...
CREATE TRIGGER `tu_cp_missing_attendance`
	...
	CALL ATTENDANCE_CALC(NEW.course_period_id, NEW.syear,NEW.institute_id);
    END IF;
    END$$
DELIMITER ;
