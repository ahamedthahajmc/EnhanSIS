...
include('../../functions/ListOutputFnc.php');

if (clean_param($_REQUEST['values'], PARAM_NOTAGS) && ($_POST['values'] || $_REQUEST['ajax'])) {
    foreach ($_REQUEST['values'] as $id => $columns) {
        if ($id != 'new') {
            $sql1 = 'SELECT TITLE,SHORT_NAME,SORT_ORDER FROM institute_gradelevels WHERE INSTITUTE_ID =\'' . UserInstitute() . '\' AND ID!=' . $id;
            ...
            $sql = 'UPDATE institute_gradelevels SET ';
            ...
            $sql = 'SELECT TITLE,SHORT_NAME,SORT_ORDER FROM institute_gradelevels WHERE INSTITUTE_ID =\'' . UserInstitute() . '\'';
            ...
            if (is_array($grd_title) && in_array($columns['TITLE'], $grd_title)) {
                ...
                if (is_array($shortname) && in_array($columns['SHORT_NAME'], $shortname) && $columns['SHORT_NAME'] != '') {
                    ...
                    if (clean_param(trim($_REQUEST['values']['new']['TITLE']), PARAM_NOTAGS) != '') {
                        $sql = 'INSERT INTO institute_gradelevels ';
                        $fields = 'INSTITUTE_ID,';
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'remove') {
    ...
    if ($has_assigned > 0) {
        ...
        if (DeletePrompt_GradeLevel('grade level')) {
            DBQuery("DELETE FROM institute_gradelevels WHERE ID='$grd_id'");
            DBQuery('UPDATE institute_gradelevels SET NEXT_GRADE_ID=NUll WHERE NEXT_GRADE_ID=' . $grd_id);
            unset($_REQUEST['modfunc']);
        }
    }
}
if ($_REQUEST['modfunc'] != 'remove') {
    $sql = 'SELECT ID,TITLE,SHORT_NAME,SORT_ORDER,NEXT_GRADE_ID FROM institute_gradelevels WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY SORT_ORDER';
    $QI = DBQuery($sql);
    ...
    if (AllowEdit()) {
        echo '<hr class="no-margin"/><div class="panel-body text-right"><INPUT id="setupGradeLvlBtn" class="btn btn-primary" type=submit value=' . _save . ' onclick="formcheck_institute_setup_grade_levels(this);"></div>';
    }
    echo '</div>';
    echo '</FORM>';
}
...
function makeGradeInput($value, $name)
{
    ...
    if (!$grades) {
        $grades_RET = DBGet(DBQuery('SELECT ID,TITLE FROM institute_gradelevels WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY SORT_ORDER'));
        if (count($grades_RET)) {
