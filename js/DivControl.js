...
function printViewSchedule(exportFileName, viewMode) {
    ...
    // win.document.write('<html><head><title>HaniIMS Student Information System</title><link rel="shortcut icon" href="../favicon.ico"></head><body><iframe src="' + dataSrc + '" style="min-height:100vh;width:100%" frameborder="0"></iframe></body></html>');
}
...
function autoset_gradelevel(thisElement) {
    ...
    if (gradelevelDataParsed[selectedMP] && Object.keys(gradelevelDataParsed[selectedMP]).length !== 0) {
        ...
        if (document.getElementById("gradeLevelHelp")) {
            window.$("#gradeLevelHelp").html(`<div id="gradeLevelHelp" class="help-block">You cannot change the Grade Level as the student is already in this Grade Level in ${gradelevelDataParsed[selectedMP].institute_name} for ${selectedMPText}.</div>`);
        }
        else {
            window.$("#gradeLevelArea").append(`<div id="gradeLevelHelp" class="help-block">You cannot change the Grade Level as the student is already in this Grade Level in ${gradelevelDataParsed[selectedMP].institute_name} for ${selectedMPText}.</div>`);
        }
    }
    else {
