...
function Prompt_Calender($title = 'Confirm', $question = '', $message = '', $pdf = '') {
    ...
    if (!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel']) {
        ...
        echo "<h4>$question</h4><FORM name=prompt_form class=\"form-horizontal no-margin\" id=prompt_form action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST>$message<hr class=\"no-margin\"/><div class=\"text-right p-t-15\"><INPUT type=submit id=\"setupCalendarBtn\" class=\"btn btn-primary\" value="._ok." onclick='formcheck_institute_setup_calender(this);'> &nbsp; <INPUT type=button class=\"btn btn-white\" name=delete_cancel value="._cancel." onclick='load_link(\"Modules.php?modname=$req_mod_name\");'></div></FORM>";
        PopTable('footer');
        return false;
    } else
        return true;
...
function Prompt_Copy_Institute($title = 'Confirm', $question = '', $message = '', $pdf = '') {
    $tmp_REQUEST = $_REQUEST;
    unset($tmp_REQUEST['delete_ok']);
    if ($pdf == true)
        $tmp_REQUEST['HaniIMS_PDF'] = true;
    ...
    if (!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel']) {
        echo '<BR>';
        PopTable('header', $title);
        echo "<h2 class=\"no-margin-top\">$question</h2><FORM class=no-margin name=prompt_form id=prompt_form action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST>$message<hr class=\"no-margin-top\"><div class=\"text-right\"><INPUT type=submit id=\"copyInstituteBtn\" class=\"btn btn-primary\" value="._ok." onclick='formcheck_institute_setup_copyinstitute(this);'>&nbsp;<INPUT type=button class=\"btn btn-default\" name=delete_cancel value="._cancel." onclick='load_link(\"Modules.php?modname=institutesetup/Calendar.php\");'></div></FORM>";
        PopTable('footer');
        return false;
    } else
        return true;
...
function Prompt_rollover($title = 'Confirm', $question = '', $message = '', $pdf = '') {
    ...
    if (!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel']) {
        ...
        echo '<p class="text-center"><span class="text-danger"><i class="icon-alert"></i> Caution : </span> '._rolloverIsAnIrreversibleProcessIfYouAreSureYouWantToProceedTypeInThe.' <BR>'._effectiveRollOverDateBelowYouCanUseTheNextInstituteYearSAttendanceStartDate.'.</p>';
        echo '<hr/>';
        echo '<div class="row">';
        ...
        echo '<div class="row">';
        echo '<div class="col-md-12"><h5 class="text-primary">'._enterNextInstituteYearSBeginAndEndDates.'</h5></div>';

        echo '<div class="col-md-6">';
        echo '<div class="form-group"><label class="col-md-4 control-label text-right">'._instituteBeginDate.'</label>';
        echo '<div class="col-md-8">' . DateInputAY('', 'roll_institute_start_date', '2') . '</div>'; //.col-md-8
        echo '</div>'; //.form-group
        echo '</div>'; //.col-md-4

        echo '<div class="col-md-6">';
        echo '<div class="form-group"><label class="col-md-4 control-label text-right">'._instituteEndDate.'</label>';
        echo '<div class="col-md-8">' . DateInputAY('', 'roll_institute_end_date', '3') . '</div>'; //.col-md-8
        echo '</div>'; //.form-group
        echo '</div>'; //.col-md-4
        ...
        $prev_st_d = DBGet(DBQuery('SELECT END_DATE FROM institute_years WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' '));
        echo '<input type=hidden id=prev_start_date value=' . $prev_st_d[1]['END_DATE'] . ' >';
        $check_ss = DBGet(DBQuery('SELECT * FROM institute_semesters WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY MARKING_PERIOD_ID'));
        if (count($check_ss) > 0) {
            $i = 4;
            $j = 4;
            $t = 0;
            ...
            foreach ($check_ss as $ss_i => $ss_d) {
                ...
                $check_sq = DBGet(DBQuery('SELECT * FROM institute_quarters WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND SEMESTER_ID=\'' . $ss_d['MARKING_PERIOD_ID'] . '\' '));
                if (count($check_sq) > 0) {
                    $q = $j + 1;
                    $q_val = '';
                    $p_val = '';
                    ...
                    foreach ($check_sq as $sq_i => $sq_d) {
                        ...
                        $check_sp = DBGet(DBQuery('SELECT * FROM institute_progress_periods WHERE SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' AND QUARTER_ID=\'' . $sq_d['MARKING_PERIOD_ID'] . '\'   '));
                        if (count($check_sp) > 0) {
        ...
        echo '<p class="text-danger"><i class="fa fa-info-circle"></i> '._theFollowingItemsWillBeRolledOverToTheNextInstituteYearUncheckTheItemSYouDoNotWantToBeRolledOverSomeItemsAreMandatoryAndCannotBeUnchecked.'</p>';
...
function GetStuListAttn(& $extra) {
    global $contacts_RET, $view_other_RET, $_HaniIMS;
    ...
    switch (User('PROFILE')) {
        case 'admin':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,ssm.INSTITUTE_ID AS LIST_INSTITUTE_ID,ssm.GRADE_ID ' . $extra['SELECT'];
...
//				
        ...
        case 'teacher':
            ...
            if ($extra['SELECT_ONLY'])
                ...
                if (Preferences('NAME') == 'Common')
                    ...
                    $sql .= 'CONCAT(s.LAST_NAME,\', \',s.FIRST_NAME,\' \',COALESCE(s.MIDDLE_NAME,\' \')) AS FULL_NAME,';
                $sql .= 's.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,ssm.INSTITUTE_ID,ssm.GRADE_ID ' . $extra['SELECT'];
                if ($_REQUEST['include_inactive'] == 'Y') {
        ...
        case 'student':
            ...
            $sql .= ' FROM students s,student_enrollment ssm ' . $extra['FROM'] . '
					WHERE ssm.STUDENT_ID=s.STUDENT_ID AND ssm.SYEAR=\'' . UserSyear() . '\' AND ssm.INSTITUTE_ID=\'' . UserInstitute() . '\' AND (\'' . DBDate() . '\' BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND \'' . DBDate() . '\'>ssm.START_DATE)) AND ssm.STUDENT_ID' . ($extra['ASSOCIATED'] ? ' IN (SELECT STUDENT_ID FROM students_join_users WHERE STAFF_ID=\'' . $extra['ASSOCIATED'] . '\')' : '=\'' . UserStudentID() . '\'');
            break;
        default:
            exit('Error');
    }
