...
function ListOutputFloat($result, $column_names, $singular = '', $plural = '', $link = false, $group = false, $options = false, $repeat_headers_options = true)
{
	...
	if ($_LIST['output'] != false) {
		...
		if ($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search'] != 'Search')) {
			...
			if ($where_message || ($singular && $plural) || (!isset($_REQUEST['HaniIMS_PDF']) && $options['search'])) {
			} else
				echo '<DIV id=LOx' . (count($column_names) + (($result_count != 0 && $cols && !isset($_REQUEST['HaniIMS_PDF'])) ? 1 : 0) + (($remove && !isset($_REQUEST['HaniIMS_PDF'])) ? 1 : 0)) . ' style="width:0; position: relative; height:0;"></DIV>';
			// END SEARCH BOX ----
			...
			if (!isset($_REQUEST['HaniIMS_PDF']) && ($stop - $start) > 10)
				echo '<THEAD>';
			if (!isset($_REQUEST['HaniIMS_PDF']))
				echo '<TR>';

			$i = 1;
			if ($remove && !isset($_REQUEST['HaniIMS_PDF']) && $result_count != 0) {
				echo "<TD class=\"subtabs\"><DIV id=LOx$i style='position: relative;'></DIV></TD>";
				$i++;
			}

			if ($result_count != 0 && $cols && !isset($_REQUEST['HaniIMS_PDF'])) {
				foreach ($column_names as $key => $value) {
			...
			for ($i = $start; $i <= $stop; $i++) {
				$item = $result[$i];
				if (isset($_REQUEST['HaniIMS_PDF']) && $options['print'] && count($item)) {
				...
				if (isset($_REQUEST['HaniIMS_PDF']) && $count % $repeat_headers == 0) {
					if ($count != 0) {
						echo '</TABLE><TABLE class="table table-bordered">';
						echo '<!-- NEW PAGE -->';
					}
					echo "<TR>";
					if ($remove && !isset($_REQUEST['HaniIMS_PDF']))
				...
				if ($cols) {
					foreach ($column_names as $key => $value) {
						if ($link[$key] && !isset($_REQUEST['HaniIMS_PDF'])) {
		...
		if ($result_count == 0) {
			...
			elseif (($link['add']['html'] || $link['add']['span']) && count($column_names) && !isset($_REQUEST['HaniIMS_PDF'])) {
				$color = $side_color;
		...
		if ($result_count != 0) {
			if ($options['yscroll']) {
				...
				if ($cols && !isset($_REQUEST['HaniIMS_PDF'])) {
					$color = $side_color;
					foreach ($result as $item) {
