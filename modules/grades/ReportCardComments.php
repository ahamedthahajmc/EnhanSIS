...
//
//
//                            $ck_sql = 'SELECT * from report_card_comments WHERE  SORT_ORDER=' . $value . ' AND INSTITUTE_ID=' . UserInstitute() . ' AND SYEAR=' . UserSyear();
//
//
//                            if ($c_id == 'NULL')
...
//                        }
                    ...
                    if (clean_param(trim($_REQUEST['values']['new']['TITLE']), PARAM_NOTAGS) != '') {
                        ...
                        $fields = 'INSTITUTE_ID,SYEAR,COURSE_ID,';
                        $values = '\'' . UserInstitute() . '\',\'' . UserSyear() . '\',' . ($_REQUEST['tab_id'] != 'new' ? "'$_REQUEST[tab_id]'" : 'NULL') . ',';
...
if (!$_REQUEST['modfunc']) {
    if (User('PROFILE') == 'admin') {
        $courses_RET = DBGet(DBQuery('SELECT TITLE,COURSE_ID FROM courses WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' AND COURSE_ID IN (SELECT DISTINCT COURSE_ID FROM course_periods WHERE GRADE_SCALE_ID IS NOT NULL) ORDER BY TITLE'));
    ...
    if ($_REQUEST['tab_id'] != 'new') {
        ...
        // need to be more specific since course_period_id=0 is not unique
            $sql = 'SELECT * FROM report_card_comments WHERE COURSE_ID=\'' . $_REQUEST['tab_id'] . '\' AND SYEAR=\'' . UserSyear() . '\' AND INSTITUTE_ID=\'' . UserInstitute() . '\' ORDER BY SORT_ORDER';
        ...
        $sql = 'SELECT * FROM report_card_comments WHERE INSTITUTE_ID=\'' . UserInstitute() . '\' AND SYEAR=\'' . UserSyear() . '\' AND COURSE_ID IS NULL ORDER BY SORT_ORDER';
